 ...
 * 
 * @param {string=}
 *            opt_value The initial value of the field. Should be in
 *            'YYYY-MM-DD' format. Defaults to the current date.
 * @param {Function=}
 *            opt_validator A function that is called to validate changes to the
 *            field's value. Takes in a date string & returns a validated date
 *            string ('YYYY-MM-DD' format), or null to abort the change.
 ...
 * 
 * @param {!Object}
 *            options A JSON object with options (date).
...
/**
 * Background colour for the dropdown div showing the date picker. Must be a CSS
 * string.
 * 
 * @type {string}
 * @private
...
/**
 * Render the field. If the picker is shown make sure it has the current date
 * selected.
 * 
 * @protected
 */
...
Blockly.FieldDate.prototype.dropdownCreate_ = function() {
    ...
    this.changeEventKey_ = goog.events.listen(picker, goog.ui.DatePicker.Events.CHANGE, this.onDateSelected_, null, this);
    this.activeMonthEventKey_ = goog.events.listen(picker, goog.ui.DatePicker.Events.CHANGE_ACTIVE_MONTH, this.updateEditor_, null, this);

    return picker;
};

/**
 * Dispose of references to DOM elements and events belonging to the date
 * editor.
 * 
 * @private
 */
...
/**
 * Load the best language pack by scanning the Blockly.Msg object for a language
 * that matches the available languages in Closure.
 * 
 * @private
 */
 ...
 */
Blockly.FieldDate.CSS = [ '.blocklyDatePicker,', '.blocklyDatePicker th,', '.blocklyDatePicker td {', '  font: 13px Arial, sans-serif;', '  color: #3c4043;',
        '}',

        '.blocklyDatePicker th,', '.blocklyDatePicker td {', '  text-align: center;', '  vertical-align: middle;', '}',

        '.blocklyDatePicker .goog-date-picker-wday,', '.blocklyDatePicker .goog-date-picker-date {', '  padding: 6px 6px;', '}',

        '.blocklyDatePicker button {', '  cursor: pointer;', '  padding: 6px 6px;', '  margin: 1px 0;', '  border: 0;', '  color: #3c4043;',
        '  font-weight: bold;', '  background: transparent;', '}',

        '.blocklyDatePicker .goog-date-picker-previousMonth,', '.blocklyDatePicker .goog-date-picker-nextMonth {', '  height: 24px;', '  width: 24px;', '}',

        '.blocklyDatePicker .goog-date-picker-monthyear {', '  font-weight: bold;', '}',

        '.blocklyDatePicker .goog-date-picker-wday, ', '.blocklyDatePicker .goog-date-picker-other-month {', '  color: #70757a;', '  border-radius: 12px;',
        '}',

        '.blocklyDatePicker button,', '.blocklyDatePicker .goog-date-picker-date {', '  cursor: pointer;', '  background-color: rgb(218, 220, 224, 0);',
        '  border-radius: 12px;', '  transition: background-color,opacity 100ms linear;', '}',

        '.blocklyDatePicker button:hover,', '.blocklyDatePicker .goog-date-picker-date:hover {', '  background-color: rgb(218, 220, 224, .5);', '}' ];
