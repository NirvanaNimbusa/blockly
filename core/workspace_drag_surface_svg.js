...
/**
 * @fileoverview An SVG that floats on top of the workspace. Blocks are moved
 *               into this SVG during a drag, improving performance. The entire
 *               SVG is translated using CSS translation instead of SVG so the
...
/**
 * Blocks are moved into this SVG during a drag, improving performance. The
 * entire SVG is translated using CSS transforms instead of SVG so the blocks
 * are never repainted during drag improving performance.
 * 
 * @param {!Element}
...
Blockly.WorkspaceDragSurfaceSvg.prototype.createDom = function() {
     ...
     * progress, the SVG is empty and display: none. <svg
     * class="blocklyWsDragSurface" style=transform:translate3d(...)> <g
     * class="blocklyBlockCanvas"></g> <g class="blocklyBubbleCanvas">/g>
     * </svg>
     */
...
Blockly.WorkspaceDragSurfaceSvg.prototype.translateSurface = function(x, y) {
    ...
    Blockly.utils.dom.setCssTransform(this.SVG_, 'translate3d(' + fixedX + 'px, ' + fixedY + 'px, 0px)');
};
 ...
 * 
 * @param {SVGElement}
 *            newSurface The element to put the drag surface contents into.
 * @package
 */
Blockly.WorkspaceDragSurfaceSvg.prototype.clearAndHide = function(newSurface) {
    ...
    if (!blockCanvas || !bubbleCanvas || !Blockly.utils.dom.hasClass(blockCanvas, 'blocklyBlockCanvas')
...
/**
 * Set the SVG to have the block canvas and bubble canvas in it and then show
 * the surface.
 * 
 * @param {!Element}
 *            blockCanvas The block canvas <g> element from the workspace.
 * @param {!Element}
 *            bubbleCanvas The <g> element that contains the bubbles.
 * @param {Element}
 *            previousSibling The element to insert the block canvas and bubble
 *            canvas after when it goes back in the DOM at the end of a drag.
 * @param {number}
 *            width The width of the workspace SVG element.
 * @param {number}
 *            height The height of the workspace SVG element.
 * @param {number}
 *            scale The scale of the workspace being dragged.
 * @package
 */
