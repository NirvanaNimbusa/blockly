...
Blockly.Trashcan = function(workspace) {
    ...
    /**
     * A list of Xml (stored as strings) representing blocks "inside" the
     * trashcan.
     * 
     * @type {Array}
     * @private
    ...
    if (this.workspace_.horizontalLayout) {
        flyoutWorkspaceOptions.toolboxPosition = this.workspace_.toolboxPosition == Blockly.TOOLBOX_AT_TOP ? Blockly.TOOLBOX_AT_BOTTOM : Blockly.TOOLBOX_AT_TOP;
        this.flyout_ = new Blockly.HorizontalFlyout(flyoutWorkspaceOptions);
    } else {
        flyoutWorkspaceOptions.toolboxPosition = this.workspace_.toolboxPosition == Blockly.TOOLBOX_AT_RIGHT ? Blockly.TOOLBOX_AT_LEFT
...
/**
 * The openness of the lid when the trashcan contains blocks. (0.0 = closed, 1.0 =
 * open)
 * 
 * @type {number}
 * @private
...
Blockly.Trashcan.prototype.createDom = function() {
    /*
     * Here's the markup that will be generated: <g class="blocklyTrash">
     * <clippath id="blocklyTrashBodyClipPath837493"> <rect width="47"
     * height="45" y="15"></rect> </clippath> <image width="64" height="92"
     * y="-32" xlink:href="media/sprites.png"
     * clip-path="url(#blocklyTrashBodyClipPath837493)"></image> <clippath
     * id="blocklyTrashLidClipPath837493"> <rect width="47" height="15"></rect>
     * </clippath> <image width="84" height="92" y="-32"
     * xlink:href="media/sprites.png"
     * clip-path="url(#blocklyTrashLidClipPath837493)"></image> </g>
     */
    ...
    body.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', this.workspace_.options.pathToMedia + Blockly.SPRITE.url);
 ...
 * 
 * @param {number}
 *            verticalSpacing Vertical distance from workspace edge to the same
 *            edge of the trashcan.
 * @return {number} Vertical distance from workspace edge to the opposite edge
 *         of the trashcan.
 */
...
/**
 * Position the trashcan. It is positioned in the opposite corner to the corner
 * the categories/toolbox starts at.
 */
Blockly.Trashcan.prototype.position = function() {
    ...
    if (metrics.toolboxPosition == Blockly.TOOLBOX_AT_LEFT || (this.workspace_.horizontalLayout && !this.workspace_.RTL)) {
    ...
    if (metrics.toolboxPosition == Blockly.TOOLBOX_AT_BOTTOM) {
        ...
        this.top_ = metrics.viewHeight + metrics.absoluteTop - (this.BODY_HEIGHT_ + this.LID_HEIGHT_) - this.verticalSpacing_;
    }
...
Blockly.Trashcan.prototype.getClientRect = function() {
    ...
    var bottom = top + this.LID_HEIGHT_ + this.BODY_HEIGHT_ + 2 * this.MARGIN_HOTSPOT_;
 ...
 * 
 * @param {number}
 *            lidAngle The angle at which to set the lid.
 * @private
 */
Blockly.Trashcan.prototype.setLidAngle_ = function(lidAngle) {
    var openAtRight = this.workspace_.toolboxPosition == Blockly.TOOLBOX_AT_RIGHT || (this.workspace_.horizontalLayout && this.workspace_.RTL);
    this.svgLid_.setAttribute('transform', 'rotate(' + (openAtRight ? -lidAngle : lidAngle) + ',' + (openAtRight ? 4 : this.WIDTH_ - 4) + ','
            + (this.LID_HEIGHT_ - 2) + ')');
};
...
Blockly.Trashcan.prototype.onDelete_ = function() {
    ...
    return function(event) {
        ...
        if (event.type == Blockly.Events.BLOCK_DELETE && event.oldXml.tagName.toLowerCase() != 'shadow') {
            ...
            if (trashcan.contents_.length > trashcan.workspace_.options.maxTrashcanContents) {
                trashcan.contents_.splice(trashcan.workspace_.options.maxTrashcanContents, trashcan.contents_.length
...
/**
 * Converts XML representing a block into text that can be stored in the content
 * array.
 * 
 * @param {!Element}
 *            xml An XML tree defining the block and any connected child blocks.
