 ...
 * 
 * @param {(!Array.
 *            <!Array>|!Function)} menuGenerator An array of options for a
 ...
 *            field's value. Takes in a language-neutral dropdown option &
 *            returns a validated language-neutral dropdown option, or null to
 *            abort the change.
 ...
 * 
 * @param {!Object}
 *            options A JSON object with options (options).
...
Blockly.FieldDropdown.prototype.widgetCreate_ = function() {
    ...
    this.menuActionEventKey_ = goog.events.listen(menu, goog.ui.Component.EventType.ACTION, this.handleMenuActionEvent_, false, this);
...
/**
 * Place the menu correctly on the screen, taking into account the dimensions of
 * the menu and the dimensions of the screen so that it doesn't run off any
 * edges.
 * 
 * @param {!goog.ui.Menu}
...
/**
 * Returns the coordinates of the anchor rectangle for the widget div. On a
 * FieldDropdown we take the top-left corner of the field, then adjust for the
 * size of the checkmark that is displayed next to the currently selected item.
 * This means that the item text will be positioned directly under the field
 * text, rather than offset slightly.
 * 
 ...
 * 
 * @param {!Array.
 *            <!Array>} options Array of option tuples: (human-readable text or
 *            image, language-neutral name).
 * @param {number}
 *            prefixLength The length of the common prefix.
 * @param {number}
 *            suffixLength The length of the common suffix
 ...
 * 
 * @return {!Array.<!Array>} Array of option tuples: (human-readable text or
 *         image, language-neutral name).
 * @throws If
 *             generated options are incorrectly structured.
 */
...
Blockly.FieldDropdown.prototype.doClassValidation_ = function(newValue) {
    ...
    if (!isValueValid) {
        if (this.sourceBlock_) {
            console.warn('Cannot set the dropdown\'s value to an unavailable option.' + ' Block type: ' + this.sourceBlock_.type + ', Field name: ' + this.name
                    + ', Value: ' + newValue);
        }
        return null;
    }
 ...
 * 
 * @param {string}
 *            newValue The new language-enutral value.
 * @protected
 */
 ...
 * 
 * @param {?}
 *            options The proposed dropdown options.
 * @throws If
 *             proposed options are incorrectly structured.
 * @private
 */
