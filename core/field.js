...
/**
 * @fileoverview Field. Used for editable titles, variables, etc. This is an
 *               abstract class that defines the UI on the block. Actual
 ...
 * @param {Function=}
 *            opt_validator A function that is called to validate changes to the
 *            field's value. Takes in a value & returns a validated value, or
 ...
 * 
 * @param {string}
 *            type The field type name as used in the JSON definition.
 * @param {!{fromJson:
 *            Function}} fieldClass The field class containing a fromJson
 *            function that can construct an instance of the field.
 * @throws {Error}
 *             if the type name is empty, or the fieldClass is not an object
...
/**
 * Construct a Field from a JSON arg object. Finds the appropriate registered
 * field by the type name as registered using Blockly.Field.register.
 * 
 * @param {!Object}
 *            options A JSON object with a type and options specific to the
 *            field type.
...
/**
 * A generic value possessed by the field. Should generally be non-null, only
 * null when the field is created.
 * 
 * @type {*}
 * @protected
 */
 ...
 * are not. Editable fields should also be serializable. This is not the case by
 * default so that SERIALIZABLE is backwards compatible.
 * 
 * @type {boolean}
 * @const
 * @default
...
/**
 * Initialize everything to render this field. Override methods initModel and
 * initView rather than this method.
 * 
 * @package
 */
 ...
 * 
 * @param {!Element}
 *            fieldElement The element containing info about the field's state.
 * @package
 */
 ...
 * 
 * @param {!Element}
 *            fieldElement The element to populate with info about the field's
 *            state.
...
Blockly.Field.prototype.isClickable = function() {
    return !!this.sourceBlock_ && this.sourceBlock_.isEditable() && !!this.showEditor_ && (typeof this.showEditor_ === 'function');
};
...
/**
 * Check whether this field should be serialized by the XML renderer. Handles
 * the logic for backwards compatibility and incongruous states.
 * 
...
Blockly.Field.prototype.isSerializable = function() {
    ...
    if (this.name) {
        if (this.SERIALIZABLE) {
            ...
            console.warn('Detected an editable field that was not serializable.' + ' Please define SERIALIZABLE property as true on all editable custom'
...
/**
 * Sets whether this editable field is visible or not. Should only be called by
 * input.setVisible.
 * 
 * @param {boolean}
 ...
 * set validator.
 * 
 * The validator function takes in the new field value, and returns validated
 * value. The validated value could be the input value, a modified version of
 * the input value, or null to abort the change.
 * 
 * If the function does not return anything (or returns undefined) the new value
 * is accepted as valid. This is to allow for fields using the validated
 * founction as a field-level change event notification.
 * 
 * @param {Function=}
 *            handler The validator function or null to clear a previous
 *            validator.
 */
 ...
 * 
 * @deprecated May 2019 Use Blockly.Field.updateSize_() to force an update to
 *             the size of the field, or Blockly.Field.getCachedWidth() to check
 *             the size of the field..
 */
Blockly.Field.prototype.updateWidth = function() {
    console.warn('Deprecated call to updateWidth, call' + ' Blockly.Field.updateSize_ to force an update to the size of the'
            + ' field, or Blockly.Field.getCachedWidth() to check the size of the' + ' field.');
    this.updateSize_();
};
 ...
 * 
 * @param {!Element}
 *            textElement An SVG 'text' element.
...
Blockly.Field.prototype.getSize = function() {
    ...
    if (this.isDirty_) {
        ...
        console.warn('Deprecated use of setting size_.width to 0 to rerender a' + ' field. Set field.isDirty_ to true instead.');
        this.render_();
    }
    return this.size_;
};
 ...
 * rerender this field and adjust for any sizing changes. Other fields on the
 * same block will not rerender, because their sizes have already been recorded.
 * 
 * @package
 */
 ...
 * Subclasses should override doClassValidation_ and doValueUpdate_ rather than
 * this method.
 * 
 * @param {*}
 *            newValue New value.
 ...
 * @private
 */
Blockly.Field.prototype.processValidation_ = function(newValue, validatedValue) {
 ...
 * 
 * @param {*}
 *            newValue The value to be validated.
 ...
 * subclasses, see FieldTextInput. No-op by default.
 * 
 * @param {*}
 *            _invalidValue The input value that was determined to be invalid.
 * @protected
 */
...
/**
 * The element to bind the click handler to. If not set explicitly, defaults to
 * the SVG root of the field. When this element is clicked on an editable field,
...
/**
 * Return the absolute coordinates of the top-left corner of this field. The
