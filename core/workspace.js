...
/**
 * Class for a workspace. This is a data structure that contains blocks. There
 * is no UI, and can be created headlessly.
 * 
 * @param {!Blockly.Options=}
 *            opt_options Dictionary of options.
 * @constructor
 */
Blockly.Workspace = function(opt_options) {
     ...
     * of the named variables in the workspace, including variables that are not
     * currently in use.
     * 
     ...
     * variable regardless of whether the variable has been created yet. A
     * FieldVariable must always refer to a Blockly.VariableModel. We reconcile
...
/**
 * Compare function for sorting objects (blocks, comments, etc) by position; top
 * to bottom (with slight LTR or RTL bias).
 * 
 * @param {!Blockly.Block |
 *            !Blockly.WorkspaceComment} a The first object to compare.
 * @param {!Blockly.Block |
 *            !Blockly.WorkspaceComment} b The second object to compare.
...
Blockly.Workspace.prototype.sortObjects_ = function(a, b) {
    ...
    return (aXY.y + Blockly.Workspace.prototype.sortObjects_.offset * aXY.x) - (bXY.y + Blockly.Workspace.prototype.sortObjects_.offset * bXY.x);
};
...
Blockly.Workspace.prototype.getTopBlocks = function(ordered) {
    ...
    if (ordered && blocks.length > 1) {
        this.sortObjects_.offset = Math.sin(Blockly.utils.math.toRadians(Blockly.Workspace.SCAN_ANGLE));
        if (this.RTL) {
...
Blockly.Workspace.prototype.getBlocksByType = function(type, ordered) {
    ...
    if (ordered && blocks.length > 1) {
        this.sortObjects_.offset = Math.sign(Blockly.utils.math.toRadians(Blockly.Workspace.SCAN_ANGLE));
        if (this.RTL) {
            this.sortObjects_.offset *= -1;
        }
        blocks.sort(this.sortObjects_);
 ...
 * 
 * @param {!Blockly.WorkspaceComment}
...
/**
 * Find all blocks in workspace. Blocks are optionally sorted by position; top
 * to bottom (with slight LTR or RTL bias).
 * 
 * @param {boolean}
 *            ordered Sort the list if true.
 ...
 * 
 * @param {string}
 *            name The name of the variable. This must be unique across
 ...
 * @param {!Array.
 *            <!Blockly.Block>} uses An array of uses of the variable.
 * @private
 */
 ...
 * @param {string=}
 *            opt_type The type of the variable. If not provided it defaults to
 ...
 * 
 * @param {?string}
 *            type Type of the variables to find.
 ...
 * compatibility in XML. Not relevant for a headless workspace.
 * 
 * @return {number} Width.
 */
 ...
 * 
 * @param {string}
 *            type Type of block to return capacity for.
 ...
 * the total remaining capacity, it returns false. If a type count is more than
 * the remaining capacity for that type, it returns false.
 * 
 * @param {!Object}
 *            typeCountsMap A map of types to counts (usually representing
 *            blocks to be created).
 * @return {boolean} True if there is capacity for the given map, false
 *         otherwise.
 */
...
/**
 * Checks if the workspace has any limits on the maximum number of blocks, or
 * the maximum number of blocks of specific types.
 * 
 ...
 * 
 * @param {boolean}
 *            redo False if undo, true if redo.
 */
...
/**
 * When something in this workspace changes, call a function. Note that there
 * may be a few recent events already on the stack. Thus the new change listener
 * might be called with events that occurred a few milliseconds before the
