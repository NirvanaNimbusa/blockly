...
/**
 * Return the coordinates of the top-left corner of this element relative to the
 * div Blockly was injected into.
 * 
 * @param {!Element}
 *            element SVG element to find the coordinates of. If this is not a
 *            child of the div Blockly was injected into, the behaviour is
 *            undefined.
 * @return {!Blockly.utils.Coordinate} Object with .x and .y properties.
 */
 ...
 * style property. Accounts for same exceptions as XY_REGEX_.
 * 
 * @type {!RegExp}
 * @private
 */
 ...
 *            svg SVG element.
 * @param {SVGMatrix}
 *            matrix Inverted screen CTM to use.
...
/**
 * Parse a string with any number of interpolation tokens (%1, %2, ...). It will
 * also replace string table references (e.g., %{bky_my_msg} and %{BKY_MY_MSG}
 * will both be replaced with the value in Blockly.Msg['MY_MSG']). Percentage
 * sign characters '%' may be self-escaped (e.g., '%%').
 * 
 * @param {string}
 *            message Text which might contain string table references and
 ...
 * 
 * @param {string|?}
 *            message Message, which may be a string that contains string table
 *            references.
...
/**
 * Validates that any %{MSG_KEY} references in the message refer to keys of the
 * Blockly.Msg string table.
 * 
 * @param {string}
 *            message Text which might contain string table references.
 ...
 *            interpolation tokens.
 * @param {boolean}
 *            parseInterpolationTokens Option to parse numeric interpolation
        ...
        if (state == 0) {
            ...
            if (c == '') {
                ...
                if (/[A-Z]\w*/i.test(rawKey)) { // Strict matching
                    ...
                    var bklyKey = Blockly.utils.string.startsWith(keyUpper, 'BKY_') ? keyUpper.substring(4) : null;
                    if (bklyKey && bklyKey in Blockly.Msg) {
                        var rawValue = Blockly.Msg[bklyKey];
                        if (typeof rawValue == 'string') {
                            // Attempt to dereference substrings, too, appending to the end.
                            Array.prototype.push.apply(tokens, Blockly.utils.tokenizeInterpolation_(rawValue, parseInterpolationTokens));
...
/**
 * Legal characters for the unique ID. Should be all on a US keyboard. No
 * characters that conflict with XML or JSON. Requests to remove additional
...
/**
 * Check if 3D transforms are supported by adding an element and attempting to
 * set the property.
 * 
 ...
 *            fn Function to run.
 * @throws Error
 *             Will throw if no global document can be found (e.g., Node.js).
 */
 ...
 * 
 * @param {!Array}
 *            arr Array from which to remove value.
 * @param {*}
...
/**
 * Gets the document scroll distance as a coordinate object. Copied from
 * Closure's goog.dom.getDocumentScroll.
 * 
...
Blockly.utils.getDocumentScroll = function() {
    ...
    return new Blockly.utils.Coordinate(win.pageXOffset || el.scrollLeft, win.pageYOffset || el.scrollTop);
};
 ...
 * @param {boolean=}
 *            opt_stripFollowing Optionally ignore all following statements
 *            (blocks that are not inside a value or statement input of the
 *            block).
