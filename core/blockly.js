...
/**
 * The main workspace most recently used. Set by
...
/**
 * Size the workspace when the contents change. This also updates scrollbars
 * accordingly.
 * 
 ...
 * change (e.g. when a block is added or removed). Record the height/width of
 * the SVG image.
 * 
 * @param {!Blockly.WorkspaceSvg}
 *            workspace Any workspace in the SVG.
 */
...
Blockly.onKeyDown_ = function(e) {
    ...
    if (mainWorkspace.options.readOnly || Blockly.utils.isTargetInput(e) || (mainWorkspace.rendered && !mainWorkspace.isVisible())) {
    ...
    if (e.keyCode == 27) {
        ...
        if (e.keyCode == 86) {
            ...
            if (Blockly.clipboardXml_) {
                ...
                if (Blockly.clipboardTypeCounts_ && workspace.isCapacityAvailable(Blockly.clipboardTypeCounts_)) {
 ...
 * 
 * @param {!Blockly.Block |
 *            !Blockly.WorkspaceComment} toCopy Block or Workspace Comment to be
 *            copied.
 * @private
 */
Blockly.copy_ = function(toCopy) {
    ...
    Blockly.clipboardTypeCounts_ = toCopy.isComment ? null : Blockly.utils.getBlockTypeCounts(toCopy, true);
};
 ...
 * 
 * @param {!Blockly.Block |
 *            !Blockly.WorkspaceComment} toDuplicate Block or Workspace Comment
 *            to be copied.
 * @private
 */
...
/**
 * Wrapper to window.alert() that app developers may override to provide
 * alternatives to the modal browser window.
 * 
 * @param {string}
 *            message The message to display to the user.
 * @param {function()=}
 *            opt_callback The callback when the alert is dismissed.
 */
...
/**
 * Wrapper to window.confirm() that app developers may override to provide
 * alternatives to the modal browser window.
 * 
 * @param {string}
 *            message The message to display to the user.
 * @param {!function(boolean)}
 *            callback The callback for handling user response.
 */
 ...
 * alternatives to the modal browser window. Built-in browser prompts are often
 * used for better text input experience on mobile device. We strongly recommend
 * testing mobile when overriding this.
 * 
 * @param {string}
 *            message The message to display to the user.
 * @param {string}
 *            defaultValue The value to initialize the prompt with.
 ...
 * 
 * @param {!Object}
 *            jsonDef The JSON definition of a block.
 * @return {function()} A function that calls jsonInit with the correct value of
 *         jsonDef.
 * @private
 */
 ...
 * 
 * @param {!Array.
 *            <!Object>} jsonArray An array of JSON block definitions.
 */
...
/**
 * Bind an event to a function call. When calling the function, verifies that it
 * belongs to the touch stream that is currently being processed, and splits
 ...
 * @param {boolean=}
 *            opt_noCaptureIdentifier True if triggering on this event should
 *            not block execution of other event handlers on this touch or other
 *            simultaneous touches. False by default.
 * @param {boolean=}
 *            opt_noPreventDefault True if triggering on this event should
 *            prevent the default handler. False by default. If
 *            opt_noPreventDefault is provided, opt_noCaptureIdentifier must
 *            also be provided.
...
Blockly.hueToHex = function(hue) {
    return Blockly.utils.colour.hsvToHex(hue, Blockly.HSV_SATURATION, Blockly.HSV_VALUE * 255);
};

/**
 * Checks old colour constants are not overwritten by the host application. If a
 * constant is overwritten, it prints a console warning directing the developer
 * to use the equivalent Msg constant.
 * 
 * @package
 */
 ...
 * 
 * @param {string}
 *            msgName The Msg constant identifier.
 * @param {Array
 *            <string>} blocklyNamePath The name parts of the tested constant.
 * @param {number|undefined}
 *            expectedValue The expected value of the constant.
 * @private
 */
Blockly.checkBlockColourConstant_ = function(msgName, blocklyNamePath, expectedValue) {
    ...
    if (value && value !== expectedValue) {
        var warningPattern = (expectedValue === undefined) ? '%1 has been removed. Use Blockly.Msg["%2"].'
 ...
 *            ws Blockly workspace to refresh theme on.
 * @private
 */
Blockly.refreshTheme_ = function(ws) {
    // Update all blocks in workspace that have a style name.
    Blockly.updateBlockStyles_(ws.getAllBlocks().filter(function(block) {
 ...
 * 
 * @param {!Array.
 *            <!Blockly.Block>} blocks List of blocks to update the style on.
 * @private
 */
