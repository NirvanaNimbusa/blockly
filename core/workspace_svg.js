 ...
 * 
 * @param {!Blockly.Options}
 *            options Dictionary of options.
 * @param {Blockly.BlockDragSurfaceSvg=}
 *            opt_blockDragSurface Drag surface for blocks.
 * @param {Blockly.WorkspaceDragSurfaceSvg=}
 *            opt_wsDragSurface Drag surface for the workspace.
...
/**
 * A wrapper function called when a resize event occurs. You can pass the result
 * to `unbindEvent_`.
 * 
 * @type {Array.<!Array>}
 */
...
/**
 * The render status of an SVG workspace. Returns `false` for headless
 * workspaces and true for instances of `Blockly.WorkspaceSvg`.
 * 
 * @type {boolean}
 */
...
/**
 * Whether this workspace has resizes enabled. Disable during batch operations
...
/**
 * Current horizontal scrolling offset in pixel units, relative to the workspace
 * origin.
 * 
 * It is useful to think about a view, and a canvas moving beneath that view. As
 * the canvas moves right, this value becomes more positive, and the view is now
 * "seeing" the left side of the canvas. As the canvas moves left, this value
 * becomes more negative, and the view is now "seeing" the right side of the
 * canvas.
 * 
 * The confusing thing about this value is that it does not, and must not
 * include the absoluteLeft offset. This is because it is used to calculate the
 * viewLeft value.
 * 
 * The viewLeft is relative to the workspace origin (although in pixel units).
 * The workspace origin is the top-left corner of the workspace (at least when
 * it is enabled). It is shifted from the top-left of the blocklyDiv so as not
 * to be beneath the toolbox.
 * 
 * When the workspace is enabled the viewLeft and workspace origin are at the
 * same X location. As the canvas slides towards the right beneath the view this
 * value (scrollX) becomes more positive, and the viewLeft becomes more negative
 * relative to the workspace origin (imagine the workspace origin as a dot on
 * the canvas sliding to the right as the canvas moves).
 * 
 * So if the scrollX were to include the absoluteLeft this would in a way
 * "unshift" the workspace origin. This means that the viewLeft would be
 * representing the left edge of the blocklyDiv, rather than the left edge of
 * the workspace.
 * 
 * @type {number}
 */
...
/**
 * Current vertical scrolling offset in pixel units, relative to the workspace
 * origin.
 * 
 * It is useful to think about a view, and a canvas moving beneath that view. As
 * the canvas moves down, this value becomes more positive, and the view is now
 * "seeing" the upper part of the canvas. As the canvas moves up, this value
 * becomes more negative, and the view is "seeing" the lower part of the canvas.
 * 
 * This confusing thing about this value is that it does not, and must not
 * include the absoluteTop offset. This is because it is used to calculate the
 * viewTop value.
 * 
 * The viewTop is relative to the workspace origin (although in pixel units).
 * The workspace origin is the top-left corner of the workspace (at least when
 * it is enabled). It is shifted from the top-left of the blocklyDiv so as not
 * to be beneath the toolbox.
 * 
 * When the workspace is enabled the viewTop and workspace origin are at the
 * same Y location. As the canvas slides towards the bottom this value (scrollY)
 * becomes more positive, and the viewTop becomes more negative relative to the
 * workspace origin (image in the workspace origin as a dot on the canvas
 * sliding downwards as the canvas moves).
 * 
 * So if the scrollY were to include the absoluteTop this would in a way
 * "unshift" the workspace origin. This means that the viewTop would be
 * representing the top edge of the blocklyDiv, rather than the top edge of the
 * workspace.
 * 
 * @type {number}
 */
...
/**
 * Whether to move workspace to the drag surface when it is dragged. True if it
 * should move, false if it should be translated directly.
 * 
 * @type {boolean}
 * @private
 */
...
/**
 * Whether the drag surface is actively in use. When true, calls to translate
 * will translate the drag surface instead of the translating the workspace
 * directly. This is set to true in setupDragSurface and to false in
 * resetDragSurface.
 * 
 * @type {boolean}
 * @private
...
/**
 * Last known position of the page scroll. This is used to determine whether we
 * have recalculated screen coordinate stuff since the page scrolled.
 * 
 * @type {!Blockly.utils.Coordinate}
 * @private
 */
 ...
 * 
 * @param {!Array.
 *            <!Object>} options List of menu options to add to.
 */
 ...
 * 
 * @return {boolean} Whether the workspace is visible. False if the workspace
 *         has been hidden by calling `setVisible(false)`.
 */
...
Blockly.WorkspaceSvg.prototype.getSvgXY = function(element) {
    ...
    var y = 0;
    var scale = 1;
    if (Blockly.utils.dom.containsNode(this.getCanvas(), element) || Blockly.utils.dom.containsNode(this.getBubbleCanvas(), element)) {
 ...
 * origin in pixels. The workspace origin is where a block would render at
 * position (0, 0). It is not the upper left corner of the workspace SVG.
 * 
...
/**
 * Return the injection div that is a parent of this workspace. Walks the DOM
 * the first time it's called, then returns a cached value.
 * 
 ...
 * 
 * @param {!Array.
 *            <!Array>} handler Data that can be passed to unbindEvent_.
 */
 ...
 * 
 * @param {string=}
 *            opt_backgroundClass Either 'blocklyMainBackground' or
 *            'blocklyMutatorBackground'.
 * @return {!Element} The workspace's SVG group.
 */
Blockly.WorkspaceSvg.prototype.createDom = function(opt_backgroundClass) {
    /**
     * <g class="blocklyWorkspace"> <rect class="blocklyMainBackground"
     * height="100%" width="100%"></rect> [Trashcan and/or flyout may go here]
     * <g class="blocklyBlockCanvas"></g> <g class="blocklyBubbleCanvas"></g>
     * </g>
     * 
     * @type {SVGElement}
     */
 ...
 * @param {string=}
 *            opt_id Optional ID. Use this ID if provided, otherwise create a
 *            new ID.
 ...
 * 
 * @param {string}
 *            tagName What type of tag the flyout belongs in.
...
/**
 * Update items that use screen coordinate calculations because something has
 * changed (e.g. scroll position, window size).
 * 
 * @private
 */
 ...
 * contents (e.g. block positions) change. This will also scroll the workspace
 * contents if needed.
 * 
 * @package
 */
...
/**
 * Resize and reposition all of the workspace chrome (toolbox, trash, scrollbars
 * etc.) This should be called when something changes that requires
 ...
 * 
 * @param {number}
 *            x Horizontal translation, in pixel units relative to the top left
 *            of the Blockly div.
 * @param {number}
 *            y Vertical translation, in pixel units relative to the top left of
 *            the Blockly div.
 */
...
/**
 * Called at the end of a workspace drag to take the contents out of the drag
 * surface and put them back into the workspace SVG. Does nothing if the
 * workspace drag surface is not enabled.
 * 
 * @package
 */
...
/**
 * Called at the beginning of a workspace drag to move contents of the workspace
 * to the drag surface. Does nothing if the drag surface is not enabled.
 * 
 * @package
 */
...
/**
 * @return {Blockly.BlockDragSurfaceSvg} This workspace's block drag surface, if
 *         one is in use.
 * @package
 */
...
/**
 * Returns the horizontal offset of the workspace. Intended for LTR/RTL
...
/**
 * Toggles the visibility of the workspace. Currently only intended for main
 * workspace.
 * 
 * @param {boolean}
 *            isVisible True if workspace should be visible.
 */
 ...
 * (for editing) were the same thing. Any calls of this function can be deleted.
 * 
 ...
 * 
 * @param {?string}
 *            id ID of block to highlight/unhighlight, or null for no block
 *            (used to unhighlight all blocks).
 * @param {boolean=}
 *            opt_state If undefined, highlight specified block and
            ...
            do {
                ...
                if (!collide) {
                    ...
                    for (var i = 0, connection; connection = connections[i]; i++) {
                        var neighbour = connection.closest(Blockly.SNAP_RADIUS, new Blockly.utils.Coordinate(blockX, blockY));
 ...
 * 
 * @param {!Element}
 *            xmlComment XML workspace comment element.
 * @private
 */
...
/**
 * Rename a variable by updating its name in the variable map. Update the flyout
 * to show the renamed variable immediately.
 * 
 * @param {string}
 *            id ID of the variable to rename.
 * @param {string}
...
/**
 * Delete a variable by the passed in ID. Update the flyout to show immediately
 * that the variable is deleted.
 * 
 * @param {string}
 *            id ID of variable to delete.
 * @package
 */
 ...
 * @param {string=}
 *            opt_type The type of the variable like 'int' or 'string'. Does not
 *            need to be unique. Field_variable can filter variables based on
 *            their type. This will default to '' which is a specific type.
 * @param {string=}
 *            opt_id The unique ID of the variable. This will default to a UUID.
...
Blockly.WorkspaceSvg.prototype.isDeleteArea = function(e) {
    if (this.deleteAreaTrash_ && this.deleteAreaTrash_.contains(e.clientX, e.clientY)) {
        return Blockly.DELETE_AREA_TRASH;
    }
    if (this.deleteAreaToolbox_ && this.deleteAreaToolbox_.contains(e.clientX, e.clientY)) {
 ...
 * @param {!Event}
 *            e Mouse down event.
 * @param {!Blockly.utils.Coordinate}
 *            xy Starting location of object.
 */
...
/**
 * Should the workspace have bounded content? Used to tell if the workspace's
 * content should be sized so that it can move (bounded) or not (exact sizing).
 * 
...
Blockly.WorkspaceSvg.prototype.isContentBounded = function() {
    return (this.options.moveOptions && this.options.moveOptions.scrollbars) || (this.options.moveOptions && this.options.moveOptions.wheel)
            || (this.options.moveOptions && this.options.moveOptions.drag) || (this.options.zoomOptions && this.options.zoomOptions.controls)
 ...
 * mouse position). This does not include zooming with the zoom controls since
 * the X Y coordinates are decided programmatically.
 * 
...
Blockly.WorkspaceSvg.prototype.isMovable = function() {
    return (this.options.moveOptions && this.options.moveOptions.scrollbars) || (this.options.moveOptions && this.options.moveOptions.wheel)
            || (this.options.moveOptions && this.options.moveOptions.drag) || (this.options.zoomOptions && this.options.zoomOptions.wheel);
};
...
/**
 * Calculate the bounding box for the blocks on the workspace. Coordinate
 * system: workspace coordinates.
 * 
 * @return {!Blockly.utils.Rect} Contains the position and size of the bounding
 *         box containing the blocks on the workspace.
 */
...
Blockly.WorkspaceSvg.prototype.showContextMenu_ = function(e) {
    ...
    if (this.options.collapse) {
         ...
         * 
         * @param {boolean}
         *            shouldCollapse Whether a block should collapse.
         * @private
         */
 ...
 * 
 * @param {number}
 *            x X coordinate of center, in pixel units relative to the top-left
 *            corner of the parentSVG.
 * @param {number}
 *            y Y coordinate of center, in pixel units relative to the top-left
 *            corner of the parentSVG.
 * @param {number}
 *            amount Amount of zooming. The formula for the new scale is
 *            newScale = currentScale * (scaleSpeed^amount). scaleSpeed is set
 *            in the workspace options. Negative amount values zoom out, and
...
Blockly.WorkspaceSvg.prototype.zoom = function(x, y, amount) {
    ...
    matrix = matrix.translate(x * (1 - scaleChange), y * (1 - scaleChange)).scale(scaleChange);
 ...
 * 
 * @param {number}
 *            type Type of zooming (-1 zooming out and 1 zooming in).
 */
 ...
 * 
 * @param {number}
 *            newScale Zoom factor. Units: (pixels / workspaceUnit).
 */
 ...
 * workspace bounds. See comment on workspaceSvg.scrollX for more detail on the
 * meaning of these values.
 * 
 * @param {number}
 *            x Target X to scroll to.
 * @param {number}
 *            y Target Y to scroll to.
 * @package
 */
Blockly.WorkspaceSvg.prototype.scroll = function(x, y) {
    ...
    if (this.scrollbar) {
        ...
        this.scrollbar.hScroll.setHandlePosition(-(x + metrics.contentLeft) * this.scrollbar.hScroll.ratio_);
        this.scrollbar.vScroll.setHandlePosition(-(y + metrics.contentTop) * this.scrollbar.vScroll.ratio_);
    }
 ...
 * 
 * @param {Blockly.Toolbox|Blockly.Flyout}
 *            elem The element to get the dimensions of, or null. It should be a
 *            toolbox or flyout, and should implement getWidth() and
 *            getHeight().
 ...
 *            ws The workspace to measure.
 * @param {!Object}
 *            svgSize An object containing height and width attributes in CSS
 *            pixels. Together they specify the size of the visible workspace,
 *            not including areas covered up by the toolbox.
 * @return {!Object} The dimensions of the contents of the given workspace, as
 *         an object containing at least - height and width in pixels - left and
 *         top in pixels relative to the workspace origin.
 * @private
 */
 ...
 * 
 * @param {!Blockly.WorkspaceSvg}
 *            ws The workspace to inspect.
 * @return {!Object} The dimensions of the contents of the given workspace, as
 *         an object containing - height and width in pixels - left, right, top
 *         and bottom in pixels relative to the workspace origin.
 * @private
 */
 ...
 *            not including areas covered up by the toolbox.
 * @return {!Object} The dimensions of the contents of the given workspace, as
 *         an object containing - height and width in pixels - left and top in
 *         pixels relative to the workspace origin.
 * @private
 */
...
/**
 * Return an object with all the metrics required to size scrollbars for a top
 * level workspace. The following properties are computed: Coordinate system:
 * pixel coordinates, -left, -up, +right, +down .viewHeight: Height of the
 * visible portion of the workspace. .viewWidth: Width of the visible portion of
 * the workspace. .contentHeight: Height of the content. .contentWidth: Width of
 * the content. .svgHeight: Height of the Blockly div (the view + the toolbox,
 * simple or otherwise), .svgWidth: Width of the Blockly div (the view + the
 * toolbox, simple or otherwise), .viewTop: Top-edge of the visible portion of
 * the workspace, relative to the workspace origin. .viewLeft: Left-edge of the
 * visible portion of the workspace, relative to the workspace origin.
 * .contentTop: Top-edge of the content, relative to the workspace origin.
 * .contentLeft: Left-edge of the content relative to the workspace origin.
 * .absoluteTop: Top-edge of the visible portion of the workspace, relative to
 * the blocklyDiv. .absoluteLeft: Left-edge of the visible portion of the
 * workspace, relative to the blocklyDiv. .toolboxWidth: Width of the toolbox,
 * if it exists. Otherwise zero. .toolboxHeight: Height of the toolbox, if it
 * exists. Otherwise zero. .flyoutWidth: Width of the flyout if it is always
 * open. Otherwise zero. .flyoutHeight: Height of the flyout if it is always
 * open. Otherwise zero. .toolboxPosition: Top, bottom, left or right. Use
 * TOOLBOX_AT constants to compare.
 * 
...
/**
 * Update whether this workspace has resizes enabled. If enabled, workspace will
 * resize when appropriate. If disabled, workspace will not resize until
 * re-enabled. Use to avoid resizing during a batch operation, for performance.
 * 
 * @param {boolean}
 *            enabled Whether resizes should be enabled.
 */
 ...
 * buttons and labels in the flyout. For instance, a button specified by the XML
 * <button text="create variable" callbackKey="CREATE_VARIABLE"></button>
 * should be matched by a call to registerButtonCallback("CREATE_VARIABLE",
 * yourCallbackFunction).
 * 
 * @param {string}
 *            key The name to use to look up this function.
 * @param {function(!Blockly.FlyoutButton)}
 *            func The function to call when the given button is clicked.
 */
 ...
 * 
 * @param {string}
 *            key The name to use to look up the function.
 ...
 * 
 * @param {string}
 *            key The name associated with the callback function.
 */
 ...
 * @param {string}
 *            key The name to use to look up this function.
 * @param {function(!Blockly.Workspace):!Array.
 *            <!Element>} func The function to call when the given toolbox
 *            category is opened.
 */
...
/**
 * Get the callback function associated with a given key, for populating custom
 * toolbox categories in this workspace.
 * 
 * @param {string}
 *            key The name to use to look up the function.
...
/**
 * Look up the gesture that is tracking this touch stream on this workspace. May
 * create a new gesture.
 * 
 * @param {!Event}
 *            e Mouse event or touch event.
