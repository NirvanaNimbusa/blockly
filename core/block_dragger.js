...
/**
 * Class for a block dragger. It moves blocks around the workspace when they are
 * being dragged by a mouse or touch.
 * 
 * @param {!Blockly.BlockSvg}
 *            block The block to drag.
 * @param {!Blockly.WorkspaceSvg}
 *            workspace The workspace to drag on.
 * @constructor
 */
Blockly.BlockDragger = function(block, workspace) {
    ...
    /**
     * Which delete area the mouse pointer is over, if any. One of
     * {@link Blockly.DELETE_AREA_TRASH}, {@link Blockly.DELETE_AREA_TOOLBOX},
     * or {@link Blockly.DELETE_AREA_NONE}.
     * 
     * @type {?number}
     * @private
     */
    ...
    /**
     * A list of all of the icons (comment, warning, and mutator) that are on
     * this block and its descendants. Moving an icon moves the bubble that
...
/**
 * Make a list of all of the icons (comment, warning, and mutator) that are on
 * this block and its descendants. Moving an icon moves the bubble that extends
 * from it if that bubble is open.
 * 
 * @param {!Blockly.BlockSvg}
 *            block The root block that is being dragged.
 ...
 *            currentDragDeltaXY How far the pointer has moved from the position
 *            at mouse down, in pixel units.
 * @param {boolean}
 *            healStack Whether or not to heal the stack after disconnecting.
 * @package
 */
...
/**
 * Shut the trash can and, if necessary, delete the dragging block. Should be
 * called at the end of a block drag.
 * 
 ...
 * correction for mutator workspaces. This function does not consider differing
 * origins. It simply scales the input's x and y values.
 * 
 * @param {!Blockly.utils.Coordinate}
 *            pixelCoord A coordinate with x and y values in CSS pixel units.
...
Blockly.BlockDragger.prototype.pixelsToWorkspaceUnits_ = function(pixelCoord) {
    var result = new Blockly.utils.Coordinate(pixelCoord.x / this.workspace_.scale, pixelCoord.y / this.workspace_.scale);
    if (this.workspace_.isMutator) {
 ...
 * 
 * @param {!Blockly.utils.Coordinate}
 *            dxy How far to move the icons from their original positions, in
 *            workspace units.
 * @private
 */
...
Blockly.BlockDragger.prototype.getInsertionMarkers = function() {
    ...
    if (this.draggedConnectionManager_ && this.draggedConnectionManager_.getInsertionMarkers) {
