...
/**
 * Class for a block's SVG representation. Not normally called directly,
 * workspace.newBlock() is preferred.
 * 
...
Blockly.BlockSvg = function(workspace, prototypeName, opt_id) {
    ...
    /**
     * Whether to move the block to the drag surface when it is dragged. True if
     * it should move, false if it should be translated directly.
     * 
     * @type {boolean}
     * @private
     */
...
/**
 * Width of this block, including any connected value blocks. Width is in
 * workspace units.
 */
...
/**
 * Map from IDs for warnings text to PIDs of functions to apply them. Used to be
 * able to maintain multiple warnings.
 * 
 ...
 * "collapsed warnings" warning without removing any warnings that belong to the
 * block.
 * 
 * @type {string}
 * @const
...
/**
 * Create and initialize the SVG representation of the block. May be called more
 * than once.
 */
 ...
 * drawing surface's origin (0,0), in workspace units. If the block is on the
 * workspace, (0, 0) is the origin of the workspace coordinate system. This does
 * not change with workspace scale.
 * 
...
/**
 * Transforms a block by setting the translation on the transform attribute of
 * the block's SVG.
 * 
 * @param {number}
 *            x The x coordinate of the translation in workspace units.
 * @param {number}
 *            y The y coordinate of the translation in workspace units.
 */
 ...
 * Generally should be called at the same time as setDragging_(true). Does
 * nothing if useDragSurface_ is false.
 * 
 * @private
 */
...
/**
 * Move this block back to the workspace block canvas. Generally should be
 * called at the same time as setDragging_(false). Does nothing if
 * useDragSurface_ is false.
 * 
 * @param {!Blockly.utils.Coordinate}
 *            newXY The position the block should take on on the workspace
 *            canvas, in workspace coordinates.
 * @private
 */
 ...
 * drag surface to translate blocks. This block must be a top-level block.
 * 
 * @param {!Blockly.utils.Coordinate}
 *            newLoc The location to translate to, in workspace coordinates.
 * @package
 */
...
/**
 * Clear the block of transform="..." attributes. Used when the block is
 * switching from 3d to 2d transform or vice versa.
 * 
 * @private
 */
 ...
 * block and any blocks stacked below it. Coordinate system: workspace
 * coordinates.
 * 
 ...
 * 
 * @param {Blockly.Field|Blockly.Block}
 *            start Current location.
 * @param {boolean}
 *            forward If true go forward, otherwise backward.
 */
 ...
 * 
 * @param {!Event}
 *            e Mouse down event or touch start event.
 * @private
 */
...
Blockly.BlockSvg.prototype.generateContextMenu = function() {
    ...
    if (!this.isInFlyout) {
        ...
        if (this.workspace.options.comments && !this.collapsed_ && this.isEditable()) {
        ...
        if (this.isMovable()) {
            if (!this.collapsed_) {
                ...
                for (var i = 1; i < this.inputList.length; i++) {
                    if (this.inputList[i - 1].type != Blockly.NEXT_STATEMENT && this.inputList[i].type != Blockly.NEXT_STATEMENT) {
 ...
 * 
 * @param {number}
 *            dx Horizontal offset from current location, in workspace units.
 * @param {number}
 *            dy Vertical offset from current location, in workspace units.
 * @private
 */
...
/**
 * Sets the colour of the border. Removes the light and dark paths if a border
 * colour is defined.
 */
...
Blockly.BlockSvg.prototype.updateDisabled = function() {
    if (!this.isEnabled() || this.getInheritedDisabled()) {
        var added = Blockly.utils.dom.addClass((this.svgGroup_), 'blocklyDisabled');
        if (added) {
            this.svgPath_.setAttribute('fill', 'url(#' + this.workspace.options.disabledPatternId + ')');
        }
    } else {
        var removed = Blockly.utils.dom.removeClass((this.svgGroup_), 'blocklyDisabled');
        if (removed) {
            this.updateColour();
        }
    }
 ...
 *            text The text, or null to delete.
 * @param {string=}
 *            opt_id An optional ID for the warning text to be able to maintain
 *            multiple warnings.
 */
Blockly.BlockSvg.prototype.setWarningText = function(text, opt_id) {
    ...
    if (collapsedParent) {
        collapsedParent.setWarningText(Blockly.Msg['COLLAPSED_WARNINGS_WARNING'], Blockly.BlockSvg.COLLAPSED_WARNING_ID);
    }
...
/**
 * Set whether the block is highlighted or not. Block highlighting is often used
 ...
 * 
 * @param {boolean}
 *            enable True if the delete cursor should be shown, false otherwise.
 * @package
 */
 ...
 * 
 * @param {number|string}
 *            colour HSV hue value, or #RRGGBB string.
 */
...
/**
 * Move this block to the front of the visible workspace. <g> tags do not
 * respect z-index so SVG renders them in the order that they are in the DOM. By
 * placing this block first within the block group's <g>, it will render on top
 * of any other blocks.
 * 
 * @package
 */
 ...
 *            for a non-rendered block return an empty list, and for a collapsed
 *            block don't return inputs connections.
