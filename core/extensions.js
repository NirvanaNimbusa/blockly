 ...
 *               adding dynamic behavior such as onchange handlers and mutators.
 *               These are applied using Block.applyExtension(), or the JSON
 *               "extensions" array attribute.
 ...
 * initialize blocks, usually adding dynamic behavior such as onchange handlers
 * and mutators. These are applied using Block.applyExtension(), or the JSON
 * "extensions" array attribute.
 * 
 * @param {string}
 *            name The name of this extension.
 * @param {Function}
 *            initFn The function to initialize an extended block.
 * @throws {Error}
...
/**
 * Registers a new extension function that adds a mutator to the block. At
 * register time this performs some basic sanity checks on the mutator. The
 * wrapper may also add a mutator dialog to the block, if both compose and
 * decompose are defined on the mixin.
 * 
 * @param {string}
 *            name The name of this mutator extension.
 * @param {!Object}
 *            mixinObj The values to mix in.
 * @param {(function())=}
 *            opt_helperFn An optional function to apply after mixing in the
 *            object.
 * @param {Array.
 *            <string>=} opt_blockList A list of blocks to appear in the flyout
 ...
 * @param {!Blockly.Block}
 *            block The block to apply the named extension to.
 * @param {boolean}
 *            isMutator True if this extension defines a mutator.
 * @throws {Error}
...
Blockly.Extensions.apply = function(name, block, isMutator) {
    ...
    if (isMutator) {
        ...
        if (!Blockly.Extensions.mutatorPropertiesMatch_(mutatorProperties, block)) {
            throw Error('Error when applying extension "' + name + '": ' + 'mutation properties changed when applying a non-mutator extension.');
        }
    }
};
 ...
 * 
 * @param {string}
 *            errorPrefix The string to prepend to any error message.
 * @param {*}
 *            func Function to check.
 * @param {string}
 *            propertyName Which property to check.
 * @throws {Error}
 *             if the property does not exist or is not a function.
 * @private
 */
Blockly.Extensions.checkHasFunction_ = function(errorPrefix, func, propertyName) {
    if (!func) {
        throw Error(errorPrefix + 'missing required property "' + propertyName + '"');
    } else if (typeof func != 'function') {
        throw Error(errorPrefix + '" required property "' + propertyName + '" must be a function');
    }
};
 ...
 * 
 * @param {string}
 *            mutationName The name of the mutation to reference in error
 *            messages.
 * @param {!Blockly.Block}
 *            block The block to check.
 * @throws {Error}
 *             if any of the properties already exist on the block.
 * @private
 */
Blockly.Extensions.checkNoMutatorProperties_ = function(mutationName, block) {
    ...
    if (properties.length) {
        throw Error('Error: tried to apply mutation "' + mutationName + '" to a block that already has mutator functions.' + '  Block id: ' + block.id);
    }
};

/**
 * Check that the given object has both or neither of the functions required to
 * have a mutator dialog. These functions are 'compose' and 'decompose'. If a
 * block has one, it must have both.
 * 
 * @param {!Object}
 ...
 *         neither function.
 * @throws {Error}
 *             if the object has only one of the functions.
 * @private
 */
Blockly.Extensions.checkMutatorDialog_ = function(object, errorPrefix) {
    ...
    if (hasCompose && hasDecompose) {
        ...
        throw Error(errorPrefix + 'Must have both or neither of "compose" and "decompose"');
    }
};
 ...
 * properties. This should be called after applying a non-mutator extension, to
 * verify that the extension didn't change properties it shouldn't.
 * 
 * @param {!Array.
 *            <Object>} oldProperties The old values to compare to.
 * @param {!Blockly.Block}
 *            block The block to inspect for new values.
 ...
 * 
 * @param {string}
 *            dropdownName The name of the field whose value is the key to the
 *            lookup table.
 * @param {!Object.
 *            <string, string>} lookupTable The table of field values to tooltip
 *            text.
 ...
 * 
 * @param {!Blockly.Block}
 *            block The block containing the dropdown
 * @param {string}
 *            dropdownName The name of the dropdown
 * @param {!Object.
 *            <string, string>} lookupTable The string lookup table
 * @private
 */
    ...
    if (!dropdown.isOptionListDynamic()) {
        ...
        for (var i = 0; i < options.length; ++i) {
            ...
            if (lookupTable[optionKey] == null) {
                console.warn('No tooltip mapping for value ' + optionKey + ' of field ' + dropdownName + ' of block type ' + block.type);
            }
        }
    }
};
...
/**
 * Builds an extension function that will install a dynamic tooltip. The tooltip
 * message should include the string '%1' and that string will be replaced with
 * the text of the named field.
 * 
 * @param {string}
 *            msgTemplate The template form to of the message text, with %1
 *            placeholder.
 * @param {string}
 *            fieldName The field with the replacement text.
 * @return {Function} The extension function.
 */
