 ...
 * @param {string}
 *            content The content of this workspace comment.
 * @param {number}
...
Blockly.WorkspaceCommentSvg = function(workspace, content, height, width, opt_id) {
    ...
    /**
     * Whether to move the comment to the drag surface when it is dragged. True
     * if it should move, false if it should be translated directly.
     * 
     * @type {boolean}
     * @private
     */
...
/**
 * Create and initialize the SVG representation of a workspace comment. May be
 ...
 * drawing surface's origin (0,0), in workspace units. If the comment is on the
...
/**
 * Transforms a comment by setting the translation on the transform attribute of
 * the block's SVG.
 * 
 * @param {number}
...
/**
 * Move this comment back to the workspace block canvas. Generally should be
 * called at the same time as setDragging(false). Does nothing if
 * useDragSurface_ is false.
 * 
 * @param {!Blockly.utils.Coordinate}
 *            newXY The position the comment should take on on the workspace
...
/**
 * Clear the comment of transform="..." attributes. Used when the comment is
 ...
 * comment. Coordinate system: workspace coordinates.
 * 
 ...
 * 
 * @param {!Element}
 *            xmlComment XML comment element.
 * @param {!Blockly.Workspace}
 *            workspace The workspace.
 * @param {number=}
 *            opt_wsWidth The width of the workspace, which is used to position
 ...
 * 
 * @param {boolean=}
 *            opt_noId True if the encoder should skip the comment ID.
