 ...
 * Example: Flyout Edge Workspace [block] / <-within this angle, drags "toward
 * workspace" | [block] ---- orthogonal to flyout boundary ---- | [block] \ |
...
/**
 * Creates the flyout's DOM. Only needs to be called once. The flyout can either
 * exist as its own svg element or be a g element nested inside a separate svg
 * element.
 * 
 * @param {string}
 *            tagName The type of tag to put the flyout in. This should be <svg>
 *            or <g>.
 * @return {!Element} The flyout's SVG group.
 */
Blockly.Flyout.prototype.createDom = function(tagName) {
    /*
     * <svg | g> <path class="blocklyFlyoutBackground"/> <g
     * class="blocklyFlyout"></g> </ svg | g>
     */
 ...
 * 
 * @param {number}
 *            width The computed width of the flyout's SVG group
 * @param {number}
 *            height The computed height of the flyout's SVG group.
 * @param {number}
 *            x The computed x origin of the flyout's SVG group.
 * @param {number}
 *            y The computed y origin of the flyout's SVG group.
 * @protected
 */
 ...
 * @param {!Element}
 *            rect The invisible rectangle under the block that acts as a mat
 *            for that block.
 * @protected
 */
...
/**
 * Does this flyout allow you to create a new instance of the given block? Used
 * for deciding if a block can be "dragged out of" the flyout.
 * 
 * @param {!Blockly.BlockSvg}
 *            block The block to copy from the flyout.
 ...
 * 
 * @param {!Blockly.BlockSvg}
 *            originalBlock The block to copy from the flyout.
 * @return {Blockly.BlockSvg} The newly created block, or null if something went
 *         wrong with deserialization.
 * @package
 */
...
/**
 * Initialize the given button: move it to the correct location, add listeners,
 * etc.
 * 
 * @param {!Blockly.FlyoutButton}
 *            button The button to initialize and place.
 * @param {number}
 *            x The x position of the cursor during this layout pass.
 * @param {number}
 *            y The y position of the cursor during this layout pass.
 * @protected
 */
 ...
 * 
 * @param {!Blockly.Block}
 *            block The block to associate the rect to.
 * @param {number}
 *            x The x position of the cursor during this layout pass.
 * @param {number}
 *            y The y position of the cursor during this layout pass.
 * @param {!{height:
 *            number, width: number}} blockHW The height and width of the block.
 * @param {number}
 *            index The index into the mats list where this rect should be
 *            placed.
 ...
 * 
 * @param {!SVGElement}
 *            rect The rectangle to move directly behind the block.
 * @param {!Blockly.BlockSvg}
 *            block The block the rectangle should be behind.
 * @protected
 */
...
Blockly.Flyout.prototype.filterForCapacity_ = function() {
    ...
    for (var i = 0, block; block = blocks[i]; i++) {
        if (this.permanentlyDisabled_.indexOf(block) == -1) {
            var enable = this.targetWorkspace_.isCapacityAvailable(Blockly.utils.getBlockTypeCounts(block));
            while (block) {
 ...
 * 
 * @param {!Blockly.Block}
 *            oldBlock The flyout block to copy.
...
Blockly.Flyout.prototype.placeNewBlock_ = function(oldBlock) {
    ...
    var oldBlockOffsetPixels = Blockly.utils.Coordinate.sum(flyoutOffsetPixels, oldBlockPos);

    // The position of the old block in pixels relative to the origin of the
    // main workspace.
    var finalOffset = Blockly.utils.Coordinate.difference(oldBlockOffsetPixels, mainOffsetPixels);
