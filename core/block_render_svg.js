 ...
 * @const
 */
Blockly.BlockSvg.TAB_PATH_DOWN = 'v 5 c 0,10 -' + Blockly.BlockSvg.TAB_WIDTH + ',-8 -' + Blockly.BlockSvg.TAB_WIDTH + ',7.5 s ' + Blockly.BlockSvg.TAB_WIDTH
 ...
 * @const
 */
Blockly.BlockSvg.TAB_PATH_DOWN_HIGHLIGHT_RTL = 'v 6.5 m -' + (Blockly.BlockSvg.TAB_WIDTH * 0.97) + ',3 q -' + (Blockly.BlockSvg.TAB_WIDTH * 0.05) + ',10 '
        + (Blockly.BlockSvg.TAB_WIDTH * 0.3) + ',9.5 m ' + (Blockly.BlockSvg.TAB_WIDTH * 0.67) + ',-1.9 v 1.4';
 ...
 * @const
 */
Blockly.BlockSvg.TOP_LEFT_CORNER_HIGHLIGHT = 'A ' + (Blockly.BlockSvg.CORNER_RADIUS - 0.5) + ',' + (Blockly.BlockSvg.CORNER_RADIUS - 0.5) + ' 0 0,1 '
...
/**
 * SVG path for drawing highlight on the top-left corner of a statement input in
 * RTL.
 * 
 * @const
 */
Blockly.BlockSvg.INNER_TOP_LEFT_CORNER_HIGHLIGHT_RTL = 'a ' + Blockly.BlockSvg.CORNER_RADIUS + ',' + Blockly.BlockSvg.CORNER_RADIUS + ' 0 0,0 '
        + (-Blockly.BlockSvg.DISTANCE_45_OUTSIDE - 0.5) + ',' + (Blockly.BlockSvg.CORNER_RADIUS - Blockly.BlockSvg.DISTANCE_45_OUTSIDE);
/**
 * SVG path for drawing highlight on the bottom-left corner of a statement input
 * in RTL.
 * 
 * @const
 */
Blockly.BlockSvg.INNER_BOTTOM_LEFT_CORNER_HIGHLIGHT_RTL = 'a ' + (Blockly.BlockSvg.CORNER_RADIUS + 0.5) + ',' + (Blockly.BlockSvg.CORNER_RADIUS + 0.5)
        + ' 0 0,0 ' + (Blockly.BlockSvg.CORNER_RADIUS + 0.5) + ',' + (Blockly.BlockSvg.CORNER_RADIUS + 0.5);
/**
 * SVG path for drawing highlight on the bottom-left corner of a statement input
 * in LTR.
 * 
 * @const
 */
Blockly.BlockSvg.INNER_BOTTOM_LEFT_CORNER_HIGHLIGHT_LTR = 'a ' + (Blockly.BlockSvg.CORNER_RADIUS + 0.5) + ',' + (Blockly.BlockSvg.CORNER_RADIUS + 0.5)
        + ' 0 0,0 ' + (Blockly.BlockSvg.CORNER_RADIUS - Blockly.BlockSvg.DISTANCE_45_OUTSIDE) + ',' + (Blockly.BlockSvg.DISTANCE_45_OUTSIDE + 0.5);
 ...
 * 
 * @param {boolean=}
 *            opt_bubble If false, just render this block. If true, also render
 *            block's parent, grandparent, etc. Defaults to true.
 */
...
Blockly.BlockSvg.prototype.renderFields_ = function(fieldList, cursorX, cursorY) {
    ...
    for (var t = 0, field; field = fieldList[t]; t++) {
        ...
        root.setAttribute('transform', 'translate(' + translateX + ',' + cursorY + ')' + scale);
...
Blockly.BlockSvg.prototype.renderDraw_ = function(iconWidth, inputRows) {
    ...
    if (this.outputConnection) {
        ...
        var renderCap = this.hat ? this.hat === 'cap' : Blockly.BlockSvg.START_HAT;
 ...
 * this render pass.
 * 
 * @param {!Blockly.BlockSvg.PathObject}
 *            pathObject The object containing partially constructed SVG paths,
...
Blockly.BlockSvg.prototype.renderDrawBottom_ = function(pathObject, cursorY) {
    ...
    if (this.squareBottomLeftCorner_) {
        ...
        if (!this.RTL) {
            ...
            highlightSteps.push('A', (Blockly.BlockSvg.CORNER_RADIUS - 0.5) + ',' + (Blockly.BlockSvg.CORNER_RADIUS - 0.5) + ' 0 0,1 ' + '0.5,'
...
Blockly.BlockSvg.prototype.renderDrawLeft_ = function(pathObject) {
    ...
    if (this.outputConnection) {
        ...
        steps.push('c 0,-10 -' + Blockly.BlockSvg.TAB_WIDTH + ',8 -' + Blockly.BlockSvg.TAB_WIDTH + ',-7.5 s ' + Blockly.BlockSvg.TAB_WIDTH + ',2.5 '
        ...
        if (this.RTL) {
            ...
            highlightSteps.push('m', (Blockly.BlockSvg.TAB_WIDTH * -0.92) + ',-0.5 q ' + (Blockly.BlockSvg.TAB_WIDTH * -0.19) + ',-5.5 0,-11');
            highlightSteps.push('m', (Blockly.BlockSvg.TAB_WIDTH * 0.92) + ',1 V 0.5 H 1');
        }
 ...
 *            which will be modified by this function.
 * @param {!Array.
 *            <!Object>} row An object containing position information about
 *            inputs on this row of the block.
 * @param {!Object}
 *            cursor An object containing the position of the cursor, which
 *            determines where to start laying out fields.
 * @private
 */
 ...
 *            determines where to start laying out fields.
 * @param {!Object}
 *            connectionPos An object containing the position of the connection
 *            on this input.
 * @param {number}
 *            rightEdge The position of the right edge of the block, which is
 *            based on the widest row that has been encountered so far.
 * @private
 */
    ...
    for (var i = 0, input; input = row[i]; i++) {
        ...
        if (input.type == Blockly.INPUT_VALUE) {
            ...
            if (this.RTL) {
                ...
                highlightInlineSteps.push('M', (cursor.x - Blockly.BlockSvg.SEP_SPACE_X - 2.5 + Blockly.BlockSvg.TAB_WIDTH - input.renderWidth) + ','
                ...
                highlightInlineSteps.push('M', (cursor.x - Blockly.BlockSvg.SEP_SPACE_X + 0.5) + ',' + (cursor.y + Blockly.BlockSvg.INLINE_PADDING_Y + 0.5));
                ...
                highlightInlineSteps.push('M', (cursor.x - input.renderWidth - Blockly.BlockSvg.SEP_SPACE_X + 0.9) + ','
                        + (cursor.y + Blockly.BlockSvg.INLINE_PADDING_Y + Blockly.BlockSvg.TAB_HEIGHT - 0.7));
                highlightInlineSteps.push('l', (Blockly.BlockSvg.TAB_WIDTH * 0.46) + ',-2.1');
            }
            // Create inline input connection.
            if (this.RTL) {
                connectionPos.x = -cursor.x - Blockly.BlockSvg.TAB_WIDTH + Blockly.BlockSvg.SEP_SPACE_X + input.renderWidth + 1;
            } else {
                connectionPos.x = cursor.x + Blockly.BlockSvg.TAB_WIDTH - Blockly.BlockSvg.SEP_SPACE_X - input.renderWidth - 1;
            }
...
Blockly.BlockSvg.prototype.renderExternalValueInput_ = function(pathObject, row, cursor, connectionPos, rightEdge) {
    ...
    if (this.RTL) {
        ...
        // Short highlight glint at bottom of tab.
        highlightSteps.push('M', (rightEdge - 5) + ',' + (cursor.y + Blockly.BlockSvg.TAB_HEIGHT - 0.7));
    ...
    if (input.connection.isConnected()) {
        this.width = Math.max(this.width, rightEdge + input.connection.targetBlock().getHeightWidth().width - Blockly.BlockSvg.TAB_WIDTH + 1);
    }
};
 ...
 *            based on the widest row that has been encountered so far.
 * @param {boolean}
 *            hasValue True if this block has at least one value input.
 * @private
 */
 ...
 *            containing position information.
 * @param {number}
 *            index The index of the current row in the inputRows array.
 * @private
 */
Blockly.BlockSvg.prototype.renderStatementInput_ = function(pathObject, row, cursor, connectionPos, inputRows, index) {
    ...
    if (this.RTL) {
        highlightSteps.push('M', (cursor.x - Blockly.BlockSvg.NOTCH_WIDTH + Blockly.BlockSvg.DISTANCE_45_OUTSIDE) + ','
 ...
 * 
 * @param {!Blockly.Block}
 *            newBlock The block to position - either the first block in a
 *            dragged stack or an insertion marker.
 * @param {!Blockly.Connection}
 *            newConnection The connection on the new block's stack - either a
 *            connection on newBlock, or the last NEXT_STATEMENT connection on
 *            the stack if the stack's being dropped before another block.
 * @param {!Blockly.Connection}
 *            existingConnection The connection on the existing block, which
 *            newBlock should line up with.
 */
Blockly.BlockSvg.prototype.positionNewBlock = function(newBlock, newConnection, existingConnection) {
    // We only need to position the new block if it's before the existing one,
    // otherwise its position is set by the previous block.
    if (newConnection.type == Blockly.NEXT_STATEMENT || newConnection.type == Blockly.INPUT_VALUE) {
 ...
 * 
 * @param {boolean}
 *            add True if highlighting should be added.
 */
