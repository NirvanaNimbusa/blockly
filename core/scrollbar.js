 ...
 * 
 * @param {number}
 *            handlePosition The value of the handle.
 * @param {number}
 *            viewSize The total size of the scrollbar's view.
 * @return {number} Ratio.
 * @private
 */
 ...
 * @param {boolean=}
 *            opt_pair True if scrollbar is part of a horiz/vert pair.
 * @param {string=}
 *            opt_class A class to be applied to this scrollbar.
 * @constructor
 */
 ...
 * the most recent drag. Units are CSS pixels, with (0, 0) at the top left of
 * the browser window. For a horizontal scrollbar this is the x coordinate of
 * the mouse down event; for a vertical scrollbar it's the y coordinate of the
 * mouse down event.
 * 
 * @type {Blockly.utils.Coordinate}
 */
...
/**
 * @param {!Object}
 *            first An object containing computed measurements of a workspace.
 * @param {!Object}
 *            second Another object containing computed measurements of a
 *            workspace.
...
Blockly.Scrollbar.metricsAreEquivalent_ = function(first, second) {
    ...
    if (first.viewWidth != second.viewWidth || first.viewHeight != second.viewHeight || first.viewLeft != second.viewLeft || first.viewTop != second.viewTop
            || first.absoluteTop != second.absoluteTop || first.absoluteLeft != second.absoluteLeft || first.contentWidth != second.contentWidth
            || first.contentHeight != second.contentHeight || first.contentLeft != second.contentLeft || first.contentTop != second.contentTop) {
        return false;
    }
 ...
 * 
 * @param {number}
 *            newLength The new scrollbar handle length in CSS pixels.
 */
 ...
 * 
 * @param {number}
 *            newPosition The new scrollbar handle offset in CSS pixels.
 */
 ...
 * 
 * @param {number}
 *            newSize The new scrollbar background length in CSS pixels.
 * @private
 */
...
Blockly.Scrollbar.prototype.resize = function(opt_metrics) {
    ...
    if (Blockly.Scrollbar.metricsAreEquivalent_(hostMetrics, this.oldHostMetrics_)) {
        return;
    }
    this.oldHostMetrics_ = hostMetrics;
 ...
 * 
 * @param {!Object}
 *            hostMetrics A data structure describing all the required
 *            dimensions, possibly fetched from the host object.
 * @private
 */
...
/**
 * Recalculate a vertical scrollbar's location within its path and length. This
 * should be called when the contents of the workspace have changed.
 * 
 * @param {!Object}
 *            hostMetrics A data structure describing all the required
 *            dimensions, possibly fetched from the host object.
...
Blockly.Scrollbar.prototype.createDom_ = function(opt_class) {
    /*
     * Create the following DOM: <svg class="blocklyScrollbarHorizontal
     * optionalClass"> <g> <rect class="blocklyScrollbarBackground" /> <rect
     * class="blocklyScrollbarHandle" rx="8" ry="8" /> </g> </svg>
     */
...
/**
 * Set whether the scrollbar's container is visible and update display
 * accordingly if visibility has changed.
 * 
 * @param {boolean}
 *            visible Whether the container is visible
 */
...
/**
 * Update visibility of scrollbar based on whether it thinks it should be
 * visible and whether its containing workspace is visible. We cannot rely on
 * the containing workspace being hidden to hide us because it is not
 * necessarily our parent in the DOM.
 */
...
/**
 * Start a dragging operation. Called when scrollbar handle is clicked.
 * 
 * @param {!Event}
 *            e Mouse down event.
 * @private
...
/**
 * Constrain the handle's position within the minimum (0) and maximum (length of
 * scrollbar) values allowed for the scrollbar.
 * 
 * @param {number}
 *            value Value that is potentially out of bounds, in CSS pixels.
 ...
 * 
 * @param {number}
 *            value The distance from the top/left end of the bar, in CSS
 *            pixels. It may be larger than the maximum allowable position of
 ...
 * used in an object whose origin isn't the same as the main workspace (e.g. in
 * a flyout.)
 * 
 * @param {number}
 *            x The x coordinate of the scrollbar's origin, in CSS pixels.
 * @param {number}
 *            y The y coordinate of the scrollbar's origin, in CSS pixels.
 */
