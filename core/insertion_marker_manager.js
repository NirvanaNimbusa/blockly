 ...
 *            block The top block in the stack being dragged.
 * @constructor
 */
Blockly.InsertionMarkerManager = function(block) {
    ...
    /**
     * The top block in the stack being dragged. Does not change during a drag.
     * 
     * @type {!Blockly.Block}
     * @private
     */
    ...
    /**
     * The workspace on which these connections are being dragged. Does not
     ...
     * first block. Set in initAvailableConnections, if at all.
     * 
     ...
     * that's not the same as the first block in the stack. Set in
     * initAvailableConnections, if at all
     * 
     ...
     * Updated on every mouse move. This is not on any of the blocks that are
     * being dragged.
     * 
     * @type {Blockly.RenderedConnection}
     * @private
    ...
    /**
     * Whether we are currently highlighting the block (shadow or real) that
     * would be replaced if the drag were released immediately.
     * 
     * @type {boolean}
     * @private
     */
...
/**
 * Connect to the closest connection and render the results. This should be
 ...
 *            dxy Position relative to drag start, in workspace units.
 * @param {?number}
 *            deleteArea One of {@link Blockly.DELETE_AREA_TRASH},
 *            {@link Blockly.DELETE_AREA_TOOLBOX}, or
 ...
 * 
 * @param {!Blockly.BlockSvg}
 *            sourceBlock The block that the insertion marker will represent.
 ...
 * only be called once, at the beginning of a drag. If the stack has more than
 * one block, this function will populate lastOnStack_ and create the
 * corresponding insertion marker.
 * 
 ...
 * 
 * @param {!Object}
 *            candidate An object containing a local connection, a closest
...
Blockly.InsertionMarkerManager.prototype.shouldUpdatePreviews_ = function(candidate, dxy) {
    ...
    if (candidateLocal && candidateClosest) {
        ...
        if (this.localConnection_ && this.closestConnection_) {
            ...
            if (this.closestConnection_ == candidateClosest && this.localConnection_ == candidateLocal) {
                return false;
            }
 ...
 *         another block. False if dropping the block immediately would result
 *         in the block being inserted in a block stack.
 * @private
 */
Blockly.InsertionMarkerManager.prototype.shouldReplace_ = function() {
    ...
    if (local.type == Blockly.OUTPUT_VALUE) {
        ...
        if (!closest.isConnected() || Blockly.Connection.lastConnectionInRow_(this.topBlock_, closest.targetConnection.getSourceBlock())) {
            return false; // Insert.
        }
 ...
 *            {@link Blockly.DELETE_AREA_TOOLBOX}, or
 *            {@link Blockly.DELETE_AREA_NONE}.
 * @return {boolean} True if dropping the block immediately would replace delete
 *         the block. False otherwise.
 * @private
 */
 ...
 * needed. At the beginning of this function, this.localConnection_ and
...
/**
 * Show an insertion marker or replacement highlighting during a drag, if
 * needed. At the end of this function, this.localConnection_ and
 * this.closestConnection_ should both be null.
 * 
 * @param {!Object}
 *            candidate An object containing a local connection, a closest
...
Blockly.InsertionMarkerManager.prototype.disconnectMarker_ = function() {
    ...
    var isFirstInStatementStack = (imConn == markerNext && !(markerPrev && markerPrev.targetConnection));

    var isFirstInOutputStack = imConn.type == Blockly.INPUT_VALUE && !(markerOutput && markerOutput.targetConnection);
...
/**
 * Get a list of the insertion markers that currently exist. Drags have 0, 1, or
 * 2 insertion markers.
 * 
