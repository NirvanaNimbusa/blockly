...
/**
 * @fileoverview Utility methods for colour manipulation. These methods are not
 *               specific to Blockly, and could be factored out into a
 *               JavaScript framework such as Closure.
...
/**
 * Parses a colour from a string. .parse('red') -> '#ff0000' .parse('#f00') ->
 * '#ff0000' .parse('#ff0000') -> '#ff0000' .parse('rgb(255, 0, 0)') ->
 * '#ff0000'
 * 
 * @param {string}
 *            str Colour in some CSS format.
 ...
 * 
 * @param {string}
 *            hexColor Colour in '#ff0000' format.
...
Blockly.utils.colour.hsvToHex = function(h, s, v) {
    ...
    return Blockly.utils.colour.rgbToHex(Math.floor(red), Math.floor(green), Math.floor(blue));
};

/**
 * Blend two colours together, using the specified factor to indicate the weight
 * given to the first colour.
 * 
 * @param {string}
 ...
 * @param {number}
 *            factor The weight to be given to colour1 over colour2. Values
 ...
 * https://www.w3.org/TR/2018/REC-css-color-3-20180619/#html4 The keys of this
 * map are the lowercase "readable" names of the colours, while the values are
 * the "hex" values.
 * 
