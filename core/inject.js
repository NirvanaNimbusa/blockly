 ...
 *            container Containing element, or its ID, or a CSS selector.
 * @param {Object=}
...
Blockly.createDom_ = function(container, options) {
    ...
    /*
     * <filter id="blocklyEmbossFilter837493"> <feGaussianBlur in="SourceAlpha"
 ...
 * 
 * @param {!Element}
 *            svg SVG element with pattern defined.
 * @param {!Blockly.Options}
 *            options Dictionary of options.
 * @param {!Blockly.BlockDragSurfaceSvg}
 *            blockDragSurface Drag surface SVG for the blocks.
 * @param {!Blockly.WorkspaceDragSurfaceSvg}
 *            workspaceDragSurface Drag surface SVG for the workspace.
 * @return {!Blockly.Workspace} Newly created main workspace.
 * @private
 */
Blockly.createMainWorkspace_ = function(svg, options, blockDragSurface, workspaceDragSurface) {
    ...
    if (!options.readOnly && !mainWorkspace.isMovable()) {
        ...
        var getWorkspaceMetrics = function() {
            ...
            workspaceMetrics.viewRight = (defaultMetrics.viewLeft + defaultMetrics.viewWidth) / scale;
            workspaceMetrics.viewBottom = (defaultMetrics.viewTop + defaultMetrics.viewHeight) / scale;
            ...
            if (mainWorkspace.isContentBounded()) {
                ...
                workspaceMetrics.contentRight = (defaultMetrics.contentLeft + defaultMetrics.contentWidth) / scale;
                workspaceMetrics.contentBottom = (defaultMetrics.contentTop + defaultMetrics.contentHeight) / scale;
            }
        ...
        var bumpObjects = function(e) {
            ...
            if (!mainWorkspace.isDragging() && !mainWorkspace.isMovable() && (Blockly.Events.BUMP_EVENTS.indexOf(e.type) != -1)) {
                var metrics = getWorkspaceMetrics();
                if (metrics.contentTop < metrics.viewTop || metrics.contentBottom > metrics.viewBottom || metrics.contentLeft < metrics.viewLeft
                    ...
                    if (e) {
                        if (!e.group) {
                            console.log('WARNING: Moved object in bounds but there was no' + ' event group. This may break undo.');
                        }
 ...
 *            mainWorkspace Newly created main workspace.
 * @private
 */
 ...
 * should not bind again. Bind events for scrolling the workspace. Most of these
 * events should be bound to the SVG's surface. However, 'mouseup' has to be on
 * the whole document so that a block dragged out of bounds and released will
 * know that it has been released. Also, 'keydown' has to be on the whole
 * document since the browser doesn't understand a concept of focus on the SVG
 * image.
 * 
 * @private
 */
 ...
 * 
 * @param {string}
 *            pathToMedia The path to the media directory.
 * @param {!Blockly.Workspace}
 *            workspace The workspace to load sounds for.
 * @private
 */
