 ...
 * 
 * @param {!Array.
 *            <!Object>} contents The blocks and buttons to lay out.
 * @param {!Array.
...
/**
 * Determine if a drag delta is toward the workspace, based on the position and
 * orientation of the flyout. This is used in determineDragIntention_ to
...
Blockly.VerticalFlyout.prototype.isDragTowardWorkspace = function(currentDragDeltaXY) {
    ...
    if ((dragDirection < range && dragDirection > -range) || (dragDirection < -180 + range || dragDirection > 180 - range)) {
        return true;
    }
    return false;
};
...
Blockly.VerticalFlyout.prototype.getClientRect = function() {
    ...
    if (this.toolboxPosition_ == Blockly.TOOLBOX_AT_LEFT) {
        ...
        if (Blockly.utils.userAgent.GECKO && this.targetWorkspace_ && this.targetWorkspace_.isMutator) {
...
/**
 * Compute width of flyout. Position mat under each block. For RTL: Lay out the
 * blocks and buttons to be right-aligned.
 * 
 * @private
 */
    ...
    if (this.width_ != flyoutWidth) {
        ...
        if (this.RTL) {
            ...
            for (var i = 0, button; button = this.buttons_[i]; i++) {
                ...
                var x = flyoutWidth / this.workspace_.scale - button.width - this.MARGIN - Blockly.BlockSvg.TAB_WIDTH;
                button.moveTo(x, y);
            }
        }
