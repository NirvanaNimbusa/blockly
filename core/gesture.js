 ...
 * 
 * @param {!Event}
 *            e The event that kicked off this gesture.
 * @param {!Blockly.WorkspaceSvg}
 *            creatorWorkspace The workspace that created this gesture and has a
 *            reference to it.
 * @constructor
 */
Blockly.Gesture = function(e, creatorWorkspace) {
    ...
    /**
     * How far the mouse has moved during this drag, in pixel units. (0, 0) is
     * at this.mouseDownXY_.
     * 
     * @type {Blockly.utils.Coordinate}
     * @private
     */
    ...
    /**
     * The block that this gesture targets. If the gesture started on a shadow
     * block, this is the first non-shadow parent of the block. If the gesture
     * started in the flyout, this is the root block of the block group that was
     * clicked or dragged.
     * 
     * @type {Blockly.BlockSvg}
     * @private
     ...
     * to the gesture, which will need to be cleared at deletion. This may be
     * different from the start workspace. For instance, a flyout is a
    ...
    /**
     * Whether the pointer has at any point moved out of the drag radius. A
     * gesture that exceeds the drag radius is a drag even if it ends exactly at
     * its start point.
     * 
     * @type {boolean}
     * @private
 ...
 * 
 * @param {!Event}
 *            e The most recent mouse or touch event.
 * @private
 */
 ...
 *            currentXY The most recent mouse/pointer position, in pixel units,
...
Blockly.Gesture.prototype.updateDragDelta_ = function(currentXY) {
    this.currentDragDeltaXY_ = Blockly.utils.Coordinate.difference(currentXY, this.mouseDownXY_);

    if (!this.hasExceededDragRadius_) {
        var currentDragDelta = Blockly.utils.Coordinate.magnitude(this.currentDragDeltaXY_);
 ...
 * flyout. This function should be called on a mouse/touch move event the first
 * time the drag radius is exceeded. It should be called no more than once per
...
/**
 * Update this gesture to record whether a bubble is being dragged. This
 * function should be called on a mouse/touch move event the first time the drag
 * radius is exceeded. It should be called no more than once per gesture. If a
 * bubble should be dragged this function creates the necessary BubbleDragger
 * and starts the drag.
 * 
...
/**
 * Update this gesture to record whether a block is being dragged. This function
 * should be called on a mouse/touch move event the first time the drag radius
 * is exceeded. It should be called no more than once per gesture. If a block
 * should be dragged, either from the flyout or in the workspace, this function
 * creates the necessary BlockDragger and starts the drag.
 * 
...
/**
 * Update this gesture to record whether a workspace is being dragged. This
 * function should be called on a mouse/touch move event the first time the drag
 * radius is exceeded. It should be called no more than once per gesture. If a
 * workspace is being dragged this function creates the necessary
...
/**
 * Update this gesture to record whether anything is being dragged. This
 * function should be called on a mouse/touch move event the first time the drag
 * radius is exceeded. It should be called no more than once per gesture.
 * 
 * @private
 */
...
/**
 * Create a bubble dragger and start dragging the selected bubble. TODO
 * (fenichel): Possibly combine this and startDraggingBlock_.
 * 
 * @private
 */
 ...
 * 
 * @param {!Event}
 *            e A mouse down or touch start event.
 * @package
 */
Blockly.Gesture.prototype.doStart = function(e) {
    ...
    if ((e.type.toLowerCase() == 'touchstart' || e.type.toLowerCase() == 'pointerdown') && e.pointerType != 'mouse') {
 ...
 * 
 * @param {!Event}
 *            e A mouse move or touch move event.
 * @package
 */
 ...
 * 
 * @param {!Event}
 *            e A mouse up or touch end event.
 * @package
 */
 ...
 *            ws The workspace the event hit.
 * @package
 */
 ...
 *            flyout The flyout the event hit.
 * @package
 */
 ...
 * @param {!Event}
 *            e A mouse down or touch start event.
 * @param {!Blockly.Bubble}
 *            bubble The bubble the event hit.
 * @package
 */
...
/*
 * End functions defining what actions to take to execute clicks on each type of
 * target.
 */
 ...
 * 
 * @param {Blockly.Field}
 *            field The field the gesture started on.
 * @package
 */
 ...
 * 
 * @param {Blockly.Bubble}
 *            bubble The bubble the gesture started on.
 * @package
 */
 ...
 * 
 * @param {Blockly.BlockSvg}
 *            block The block the gesture started on.
 * @package
 */
 ...
 * 
 * @param {Blockly.BlockSvg}
 *            block The block the gesture targets.
 * @private
 */
 ...
 * 
 * @param {Blockly.WorkspaceSvg}
 *            ws The workspace the gesture started on.
 * @private
 */
 ...
 * 
 * @param {Blockly.Flyout}
 *            flyout The flyout the gesture started on.
 * @private
 */
...
/*
 * Begin helper functions defining types of clicks. Any developer wanting to
 * change the definition of a click should modify only this code.
 */
...
Blockly.Gesture.prototype.isFieldClick_ = function() {
    var fieldClickable = this.startField_ ? this.startField_.isClickable() : false;
    return fieldClickable && !this.hasExceededDragRadius_ && (!this.flyout_ || !this.flyout_.autoClose);
};
...
/**
 * Whether this gesture is a drag of either a workspace or block. This function
 * is called externally to block actions that cannot be taken mid-drag (e.g.
 * using the keyboard to delete the selected blocks).
 * 
...
/**
 * Whether this gesture has already been started. In theory every mouse down has
 * a corresponding mouse up, but in reality it is possible to lose a mouse up,
 * leaving an in-process gesture hanging.
 * 
 * @return {boolean} Whether this gesture was a click on a workspace.
 * @package
 */
...
/**
 * Get a list of the insertion markers that currently exist. Block drags have 0,
 * 1, or 2 insertion markers.
 * 
