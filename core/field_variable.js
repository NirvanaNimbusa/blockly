 ...
 *            field's value. Takes in a variable ID & returns a validated
 *            variable ID, or null to abort the change.
 * @param {Array.
 *            <string>=} opt_variableTypes A list of the types of variables to
 *            include in the dropdown.
 * @param {string=}
 *            opt_defaultType The type of variable to create if this field's
...
/**
 * Construct a FieldVariable from a JSON arg object, dereferencing any string
 * table references.
 * 
 * @param {!Object}
 *            options A JSON object with options (variable, variableTypes, and
 *            defaultType).
 ...
 * 
 * @param {!Element}
 *            fieldElement The element containing information about the variable
 *            field's state.
 */
Blockly.FieldVariable.prototype.fromXml = function(fieldElement) {
    ...
    var variableType = fieldElement.getAttribute('variabletype') || fieldElement.getAttribute('variableType') || '';

    var variable = Blockly.Variables.getOrCreateVariablePackage(this.workspace_, id, variableName, variableType);

    // This should never happen :)
    if (variableType != null && variableType !== variable.type) {
        throw Error('Serialized variable type with id \'' + variable.getId() + '\' had type ' + variable.type + ', and '
                + 'does not match variable field that references it: ' + Blockly.Xml.domToText(fieldElement) + '.');
    }
...
/**
 * Get the variable model for the selected variable. Not guaranteed to be in the
 * variable map on the workspace (e.g. if accessed after the variable has been
 * deleted).
 * 
...
/**
 * Gets the validation function for this field, or null if not set. Returns null
 * if the variable is not set, because validators should not run on the initial
 * setValue call, because the field won't be attached to a block and workspace
 * at that point.
 * 
 ...
 * 
 * @param {string}
 *            newId The id of the new variable to set.
 ...
 * 
 * @param {string}
 *            newId The id of the new variable.
 * @protected
 */
 ...
 * @throws {Error}
 *             if variableTypes is an empty array.
 * @private
 */
 ...
 * @param {Array.
 *            <string>=} opt_variableTypes A list of the types of variables to
 *            include in the dropdown. If null or undefined, variables of all
 *            types will be displayed in the dropdown.
 * @param {string=}
 *            opt_defaultType The type of the variable to create if this field's
 *            value is not explicitly set. Defaults to ''.
 * @private
 */
Blockly.FieldVariable.prototype.setTypes_ = function(opt_variableTypes, opt_defaultType) {
    ...
    if (opt_variableTypes == null || opt_variableTypes == undefined) {
        ...
        if (!isInArray) {
            throw Error('Invalid default type \'' + defaultType + '\' in ' + 'the definition of a FieldVariable');
        }
    } else {
...
/**
 * Handle the selection of an item in the variable dropdown menu. Special case
 * the 'Rename variable...' and 'Delete variable...' options. In the rename
 * case, prompt the user for a new name.
 * 
 * @param {!goog.ui.Menu}
 *            menu The Menu component clicked.
 * @param {!goog.ui.MenuItem}
