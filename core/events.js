...
/**
 * List of events that cause objects to be bumped back into the visible portion
 * of the workspace (only used for non-movable workspaces).
 * 
 * Not to be confused with bumping so that disconnected connections to do not
 * appear connected.
 * 
 * @const
 */
 ...
 * 
 * @param {!Array.
 *            <!Blockly.Events.Abstract>} queueIn Array of events.
 * @param {boolean}
 *            forward True if forward (redo), false if backward (undo).
    ...
    for (var i = 0, event; event = queue[i]; i++) {
        if (!event.isNull()) {
            ...
            if (!lastEntry) {
                ...
                lastEntry.index = i;
            } else if (event.type == Blockly.Events.CHANGE && event.element == lastEvent.element && event.name == lastEvent.name) {
                // Merge change events.
...
/**
 * Modify pending undo events so that when they are fired they don't land in the
 * undo stack. Called by Blockly.Workspace.clearUndo.
 */
 ...
 *            state True to start new group, false to end group. String to set
 *            group explicitly.
 */
 ...
 *            workspace Target workspace for event.
...
/**
 * Enable/disable a block depending on whether it is properly connected. Use
 * this on applications where all blocks should be connected to a top block.
 * Recommend setting the 'disable' option to 'false' in the config so that users
 * don't try to re-enable disabled orphan blocks.
 * 
 * @param {!Blockly.Events.Abstract}
 *            event Custom data for event.
 */
