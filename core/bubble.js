 ...
 *            anchorXY Absolute position of bubble's anchor point.
 * @param {?number}
 ...
 * @param {!Event} e Mouse up event. @private
 */
...
/**
 * Relative X coordinate of bubble with respect to the anchor's centre, in
 * workspace units. In RTL mode the initial value is negated.
 * 
 * @private
 */
...
Blockly.Bubble.prototype.createDom_ = function(content, hasResize) {
     ...
     * filter="url(#blocklyEmbossFilter837493)"> <path d="... Z" /> <rect
    ...
    var bubbleEmboss = Blockly.utils.dom.createSvgElement('g', filter, this.bubbleGroup_);
...
Blockly.Bubble.prototype.resizeMouseDown_ = function(e) {
    ...
    this.workspace_.startDrag(e, new Blockly.utils.Coordinate(this.workspace_.RTL ? -this.width_ : this.width_, this.height_));
...
Blockly.Bubble.prototype.layoutBubble_ = function() {
    ...
    var mostOverlap = Math.max(topPositionOverlap, startPositionOverlap, closerPositionOverlap, fartherPositionOverlap);
 ...
 * 
 * @param {!Object}
 *            relativeMin The position of the top-left corner of the bubble
 *            relative to the anchor point.
 * @param {number}
 *            relativeMin.x The x-position of the relativeMin.
 * @param {number}
 *            relativeMin.y The y-position of the relativeMin.
 * @param {!Object}
 *            metrics The metrics of the workspace the bubble will appear in.
...
Blockly.Bubble.prototype.getOverlap_ = function(relativeMin, metrics) {
    ...
    var bubbleMin = {
        x : this.workspace_.RTL ? (this.anchorXY_.x - relativeMin.x - this.width_) : (relativeMin.x + this.anchorXY_.x),
    ...
    var overlapWidth = Math.min(bubbleMax.x, workspaceMax.x) - Math.max(bubbleMin.x, workspaceMin.x);
    var overlapHeight = Math.min(bubbleMax.y, workspaceMax.y) - Math.max(bubbleMin.y, workspaceMin.y);
    return Math.max(0, Math.min(1, (overlapWidth * overlapHeight) / (this.width_ * this.height_)));
};

/**
 * Calculate what the optimal horizontal position of the top-left corner of the
 * bubble is (relative to the anchor point) so that the most area of the bubble
 * is shown.
 * 
 * @param {!Object}
 *            metrics The metrics of the workspace the bubble will appear in.
 * @return {number} The optimal horizontal position of the top-left corner of
 *         the bubble.
 * @private
 */
...
/**
 * Calculate what the optimal vertical position of the top-left corner of the
 * bubble is (relative to the anchor point) so that the most area of the bubble
 * is shown.
 * 
 * @param {!Object}
 *            metrics The metrics of the workspace the bubble will appear in.
 * @return {number} The optimal vertical position of the top-left corner of the
 *         bubble.
 * @private
 */
...
/**
 * Move this bubble during a drag, taking into account whether or not there is a
 * drag surface.
 * 
 * @param {Blockly.BlockDragSurfaceSvg}
 *            dragSurface The surface that carries rendered items during a drag,
...
/**
 * Set whether auto-layout of this bubble is enabled. The first time a bubble is
 * shown it positions itself to not cover any blocks. Once a user has dragged it
 * to reposition, it renders where the user put it.
 * 
 * @param {boolean}
 *            enable True if auto-layout should be enabled, false otherwise.
 * @package
 */
