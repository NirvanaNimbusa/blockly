...
/**
 * Find all user-created variables that are in use in the workspace. For use by
 * generators. To get a list of all variables on a workspace, including unused
 * variables, call Workspace.getAllVariables.
 * 
 * @param {!Blockly.Workspace}
 *            ws The workspace to search for variables.
 ...
 * only their names. For use by generators. To get a list of all variables on a
 * workspace, including unused variables, call Workspace.getAllVariables.
 * 
...
/**
 * Find all developer variables used by blocks in the workspace. Developer
 * variables are never shown to the user, but are declared as global variables
 * in the generated code. To declare developer variables, define the
 * getDeveloperVariables function on your block and return a list of variable
 * names. For use by generators.
 * 
 * @param {!Blockly.Workspace}
...
Blockly.Variables.flyoutCategoryBlocks = function(workspace) {
    ...
    if (variableModelList.length > 0) {
        ...
        if (Blockly.Blocks['math_change']) {
            ...
            block.appendChild(Blockly.Variables.generateVariableFieldDom(mostRecentVariable));
            var value = Blockly.Xml.textToDom('<value name="DELTA">' + '<shadow type="math_number">' + '<field name="NUM">1</field>' + '</shadow>' + '</value>');
 ...
 * If no unique name is located it will try 'i' to 'z', 'a' to 'h', then 'i2' to
 * 'z2' etc. Skip 'l'.
 * 
 * @param {!Blockly.Workspace}
 *            workspace The workspace to be unique in.
 ...
 * 
 * @param {!Blockly.Workspace}
 *            workspace The workspace on which to create the variable.
 * @param {function(?string=)=}
 *            opt_callback A callback. It will be passed an acceptable new
 *            variable name, or null if change is to be aborted (cancel button),
 *            or undefined if an existing variable was chosen.
 * @param {string=}
 *            opt_type The type of the variable like 'int', 'string', or ''.
 ...
 * 
 * @param {!Blockly.Workspace}
 *            workspace The workspace on which to rename the variable.
 ...
 * 
 * @param {string}
 *            promptText The string of the prompt.
 * @param {string}
 *            defaultText The default value to show in the prompt's field.
 * @param {function(?string)}
 *            callback A callback. It will return the new variable name, or null
 ...
 * @param {string}
 *            type The type to exclude from the search.
 * @param {!Blockly.Workspace}
 *            workspace The workspace to search for the variable.
 ...
 * 
 * @param {!Blockly.VariableModel}
 *            variableModel The variable model to represent.
 * @return {Element} The generated DOM.
 * @public
 */
Blockly.Variables.generateVariableFieldDom = function(variableModel) {
    /*
     * Generates the following XML: <field name="VAR" id="goKTKmYJ8DhVHpruv"
     * variabletype="int">foo</field>
     */
...
/**
 * Helper function to look up or create a variable on the given workspace. If no
 * variable exists, creates and returns it.
 * 
 * @param {!Blockly.Workspace}
 *            workspace The workspace to search for the variable. It may be a
 *            flyout workspace or main workspace.
 * @param {string}
 *            id The ID to use to look up or create the variable, or null.
 * @param {string=}
 *            opt_name The string to use to look up or create the variable.
 * @param {string=}
 *            opt_type The type to use to look up or create the variable.
...
/**
 * Look up a variable on the given workspace. Always looks in the main workspace
 * before looking in the flyout workspace. Always prefers lookup by ID to lookup
 * by name + type.
 * 
 * @param {!Blockly.Workspace}
 *            workspace The workspace to search for the variable. It may be a
 *            flyout workspace or main workspace.
 * @param {string}
 *            id The ID to use to look up the variable, or null.
 * @param {string=}
 *            opt_name The string to use to look up the variable. Only used if
 *            lookup by ID fails.
 * @param {string=}
 *            opt_type The type to use to look up the variable. Only used if
 *            lookup by ID fails.
 * @return {Blockly.VariableModel} The variable corresponding to the given ID or
 *         name + type combination, or null if not found.
 * @public
 */
 ...
 * 
 * @param {!Blockly.Workspace}
 *            workspace The workspace in which to create the variable. It may be
 *            a flyout workspace or main workspace.
 * @param {string}
 *            id The ID to use to create the variable, or null.
 * @param {string=}
 *            opt_name The string to use to create the variable.
 * @param {string=}
 *            opt_type The type to use to create the variable.
 * @return {!Blockly.VariableModel} The variable corresponding to the given ID
 *         or name + type combination.
 * @private
 */
 ...
 * @param {!Array.
 *            <!Blockly.VariableModel>} originalVariables The array of variables
 *            that existed in the workspace before adding the new block.
 * @return {!Array.<!Blockly.VariableModel>} The new array of variables that
 *         were freshly added to the workspace after creating the new block, or []
 *         if no new variables were added to the workspace.
 * @package
 */
