...
/**
 * Class for one block. Not normally called directly, workspace.newBlock() is
 * preferred.
 * 
...
/**
 * Optional text data that round-trips beween blocks and XML. Has no effect. May
 * be used by 3rd parties for meta information.
 * 
 * @type {?string}
 */
...
/**
 * Colour of the block as HSV hue value (0-360) This may be null if the block
 * colour was not set via a hue number.
 * 
 * @type {?number}
 * @private
 */
...
/**
 * Secondary colour of the block. Colour of shadow blocks.
 * 
 * @type {?string}
 * @private
 */
...
/**
 * Tertiary colour of the block. Colour of the block's border.
 * 
 * @type {?string}
 * @private
 */
 ...
 * the first interaction with it. Interactions include UI actions (e.g. clicking
 * and dragging) and firing events (e.g. create, delete, and change).
 * 
 * @public
 */
 ...
 * 
 * @param {boolean=}
 *            opt_healStack Disconnect child statement and reconnect stack.
 *            Defaults to false.
 */
 ...
 * 
 * @param {boolean=}
 *            opt_healStack Disconnect right-side block and connect to left-side
 ...
 * attached, the connected block is attached to the insertion marker. Since only
 * one block can be displaced and attached to the insertion marker this should
 * only ever return one connection.
 * 
...
Blockly.Block.prototype.getOnlyValueConnection_ = function() {
    ...
    for (var i = 0; i < this.inputList.length; i++) {
        ...
        if (thisConnection && thisConnection.type == Blockly.INPUT_VALUE && thisConnection.targetConnection) {
            if (connection) {
...
/**
 * Unplug this statement block from its superior block. Optionally reconnect the
 * block underneath with the block on top.
 * 
 * @param {boolean=}
 *            opt_healStack Disconnect child statement and reconnect stack.
 *            Defaults to false.
 ...
 * 
 * @param {boolean}
 *            _all If true, return all connections even hidden ones.
 ...
 * 
 * @param {!Blockly.Block}
 *            block A block connected to an input on this block.
 ...
 * and statement inputs, as well as any following statement. Excludes any
 * connection on an output tab or any preceding statement. Blocks are optionally
 ...
 * Includes this block in the list. Includes value and statement inputs, as well
 * as any following statements. Excludes any connection on an output tab or any
 * preceding statements. Blocks are optionally sorted by position; top to
 * bottom.
 * 
 * @param {boolean}
 *            ordered Sort the list if true.
 ...
 * not duplicatable. If duplicating this block and descendants will put any type
 * over their maxInstances this block is not duplicatable.
 * 
...
/**
 * Set whether this block is an insertion marker block or not. Once set this
 * cannot be unset.
 * 
 * @param {boolean}
 *            insertionMarker True if an insertion marker.
 * @package
 */
 ...
 * 
 * @param {boolean}
 *            hidden True if connections are hidden.
 */
...
/**
 * Find the connection on this block that corresponds to the given connection on
 * the other block. Used to match connections between a block and its insertion
 * marker.
 * 
 * @param {!Blockly.Block}
 *            otherBlock The other block to match against.
 * @param {!Blockly.Connection}
 *            conn The other connection to match.
 ...
 * 
 * @return {{colourDark, colourLight, colourBorder}} An object containing colour
 *         values for the border(s) of the block. If the block is using a style
 *         the colourBorder will be defined and equal to the tertiary colour of
 *         the style (#RRGGBB string). Otherwise the colourDark and colourLight
 ...
 * 
 * @param {number|string}
 *            colour HSV hue value (0 to 360), #RRGGBB string, or a message
 ...
 * 
 * @param {string}
 *            blockStyleName Name of the block style
 * @throws {Error}
...
Blockly.Block.prototype.setStyle = function(blockStyleName) {
    ...
    if (!theme) {
        throw Error('Trying to set block style to ' + blockStyleName + ' before theme was defined via Blockly.setTheme().');
    }
 ...
 * 
 * @param {function(Blockly.Events.Abstract)}
 *            onchangeFn The callback to call when the block's workspace
 *            changes.
 * @throws {Error}
 ...
 * 
 * @param {!Blockly.VariableModel}
 *            variable The variable being renamed.
 * @package
 */
...
/**
 * Notification that a variable is renaming. If the ID matches one of this
 * block's variables, rename it.
 * 
 * @param {string}
 *            oldId ID of variable to rename.
 * @param {string}
 *            newId ID of new variable. May be the same as oldId, but with an
 *            updated name.
 */
 ...
 *            types. Null/undefined if any type could be connected.
 */
 ...
 * @param {(string|Array.
 *            <string>|null)=} opt_check Returned type or list of returned
 *            types. Null or undefined if any type could be returned (e.g.
 *            variable get).
 */
 ...
 * @param {string=}
 *            opt_emptyToken The placeholder string used to denote an empty
 *            field. If not specified, '?' is used.
 ...
 * 
 * @param {!Object}
 *            json Structured data describing the block.
 */
Blockly.Block.prototype.jsonInit = function(json) {
    ...
    if (typeof json['extensions'] == 'string') {
        console.warn(warningPrefix + 'JSON attribute \'extensions\' should be an array of' + ' strings. Found raw string in JSON for \'' + json['type']
                + '\' block.');
 ...
 *            json Structured data describing the block.
 * @param {string}
 *            warningPrefix Warning prefix string identifying block.
 * @private
 */
 ...
 * 
 * @param {!Object}
 *            mixinObj The key/values pairs to add to this block object.
 * @param {boolean=}
 *            opt_disableCheck Option flag to disable overwrite checks.
 */
 ...
 * 
 * @param {string}
 *            message Text contains interpolation tokens (%1, %2, ...) that
 *            match with fields or inputs defined in the args array.
 * @param {!Array}
 *            args Array of arguments to be interpolated.
 * @param {string|undefined}
 *            lastDummyAlign If a dummy input is added at the end, how should it
 *            be aligned?
 * @private
 */
Blockly.Block.prototype.interpolate_ = function(message, args, lastDummyAlign) {
    ...
    if (indexCount != args.length) {
        throw Error('Block "' + this.type + '": ' + 'Message does not reference all ' + args.length + ' arg(s).');
    }
    // Add last dummy input if needed.
    if (elements.length
            && (typeof elements[elements.length - 1] == 'string' || Blockly.utils.string.startsWith(elements[elements.length - 1]['type'], 'field_'))) {
        var dummyInput = {
            type : 'input_dummy'
        };
        if (lastDummyAlign) {
                    ...
                    default:
                        ...
                        if (!field) {
                            if (element['alt']) {
                                ...
                                console.warn('Blockly could not create a field of type ' + element['type']
 ...
 * 
 * @param {?string}
 *            _text The text, or null to delete.
 * @param {string=}
 *            _opt_id An optional ID for the warning text to be able to maintain
 *            multiple warnings.
 */
 ...
 *            _mutator A mutator dialog instance or null to remove.
 */
 ...
 * 
 * @param {number}
 *            dx Horizontal offset, in workspace units.
 * @param {number}
 *            dy Vertical offset, in workspace units.
 */
 ...
 * 
 * @param {number}
 *            type The type of the connection to create.
 ...
 * 
 * @param {boolean=}
 *            opt_shadowBlocksAreFilled An optional argument controlling whether
