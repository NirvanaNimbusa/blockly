 ...
 * 
 * @param {string=}
 *            opt_value The initial value of the field. Should cast to a string.
 *            Defaults to an empty string if null or undefined.
 * @param {Function=}
 *            opt_validator A function that is called to validate changes to the
 *            field's value. Takes in a string & returns a validated string, or
...
Blockly.FieldTextInput = function(opt_value, opt_validator) {
    ...
    Blockly.FieldTextInput.superClass_.constructor.call(this, opt_value, opt_validator);
};
goog.inherits(Blockly.FieldTextInput, Blockly.Field);

/**
 * Construct a FieldTextInput from a JSON arg object, dereferencing any string
 * table references.
 * 
 * @param {!Object}
 *            options A JSON object with options (text, class, and spellcheck).
...
/**
 * Called by setValue if the text input is not valid. If the field is currently
 * being edited it reverts value of the field to the previous value while
 * allowing the display text to be handled by the htmlInput_.
 * 
 * @param {*}
 *            _invalidValue The input value that was determined to be invalid.
 *            This is not used by the text input because its display value is
 *            stored on the htmlInput_.
 * @protected
 */
Blockly.FieldTextInput.prototype.doValueInvalid_ = function(_invalidValue) {
    if (this.isBeingEdited_) {
        ...
        if (this.sourceBlock_ && Blockly.Events.isEnabled()) {
            Blockly.Events.fire(new Blockly.Events.BlockChange(this.sourceBlock_, 'field', this.name, oldValue, this.value_));
        }
    }
};
 ...
 * field, and updates the text of the field if it is not currently being edited
 * (i.e. handled by the htmlInput_).
 * 
 * @param {string}
 *            newValue The new validated value of the field.
 * @protected
 */
...
/**
 * Updates the colour of the htmlInput given the current validity of the field's
 * value.
 * 
 * @protected
 */
...
Blockly.FieldTextInput.prototype.showEditor_ = function(opt_quietInput) {
    ...
    if (!quietInput && (Blockly.utils.userAgent.MOBILE || Blockly.utils.userAgent.ANDROID || Blockly.utils.userAgent.IPAD)) {
 ...
 * 
 * @param {boolean}
 *            quietInput True if editor should be created without focus.
 * @private
 */
...
/**
 * Close the editor, save the results, and dispose any events bound to the text
 * input's editor.
 * 
 * @private
 */
 ...
 *            htmlInput The htmlInput to which event handlers will be bound.
 * @private
 */
Blockly.FieldTextInput.prototype.bindInputEvents_ = function(htmlInput) {
    // Trap Enter without IME and Esc to hide.
    this.onKeyDownWrapper_ = Blockly.bindEventWithChecks_(htmlInput, 'keydown', this, this.onHtmlInputKeyDown_);
    // Resize after every keystroke.
    this.onKeyUpWrapper_ = Blockly.bindEventWithChecks_(htmlInput, 'keyup', this, this.onHtmlInputChange_);
    // Repeatedly resize when holding down a key.
    this.onKeyPressWrapper_ = Blockly.bindEventWithChecks_(htmlInput, 'keypress', this, this.onHtmlInputChange_);
