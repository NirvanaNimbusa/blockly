...
/**
 * Whether touch is enabled in the browser. Copied from Closure's
 * goog.events.BrowserFeature.TOUCH_ENABLED
 */
Blockly.Touch.TOUCH_ENABLED = ('ontouchstart' in Blockly.utils.global
        || !!(Blockly.utils.global['document'] && document.documentElement && 'ontouchstart' in document.documentElement) ||
// IE10 uses non-standard touch events, so it has a different check.
!!(Blockly.utils.global['navigator'] && (Blockly.utils.global['navigator']['maxTouchPoints'] || Blockly.utils.global['navigator']['msMaxTouchPoints'])));
 ...
 * which after about a second opens the context menu. The tasks is killed if the
 * touch event terminates early.
 * 
 * @param {!Event}
 *            e Touch start event.
 * @param {Blockly.Gesture}
 *            gesture The gesture that triggered this longStart.
 * @private
 */
...
/**
 * Nope, that's not a long-press. Either touchend or touchcancel was fired, or a
 * drag hath begun. Kill the queued long-press task.
 * 
 * @private
 */
...
/**
 * Decide whether Blockly should handle or ignore this event. Mouse and touch
 * events require special checks because we only want to deal with one touch
 * stream at a time. All other events should always be handled.
 * 
 * @param {!Event}
...
Blockly.Touch.getTouchIdentifierFromEvent = function(e) {
    return e.pointerId != undefined ? e.pointerId
            : (e.changedTouches && e.changedTouches[0] && e.changedTouches[0].identifier !== undefined && e.changedTouches[0].identifier !== null) ? e.changedTouches[0].identifier
                    : 'mouse';
};
 ...
 * we'll use the identifier "mouse". This means we won't deal well with multiple
 * mice being used at the same time. That seems okay. If the current identifier
 * was unset, save the identifier from the event. This starts a drag/gesture,
 * during which touch events with other identifiers will be silently ignored.
 * 
 * @param {!Event}
 *            e Mouse event or touch event.
...
Blockly.Touch.isMouseOrTouchEvent = function(e) {
    return Blockly.utils.string.startsWith(e.type, 'touch') || Blockly.utils.string.startsWith(e.type, 'mouse')
...
Blockly.Touch.isTouchEvent = function(e) {
    return Blockly.utils.string.startsWith(e.type, 'touch') || Blockly.utils.string.startsWith(e.type, 'pointer');
};
 ...
 * 
 * @param {!Event}
 *            e A mouse event or a touch event with one or more changed touches.
