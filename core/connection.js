...
/**
 * Connection database for connections compatible with this type on the current
 * workspace.
 * 
 * @type {Blockly.ConnectionDB}
 * @protected
 ...
 *            childConnection Connection on inferior block.
 * @protected
 */
 ...
 * 
 * @param {Blockly.Connection}
 *            target Connection to check compatibility with.
...
/**
 * Check if the two connections can be dragged to connect to each other. This is
 * used by the connection database when searching for the closest connection.
 * 
 * @param {!Blockly.Connection}
 *            candidate A nearby connection to check, which must be a previous
 *            connection.
 ...
 * 
 * @param {!Blockly.Connection}
 *            candidate A nearby connection to check.
 * @return {boolean} True if the connection is allowed, false otherwise.
 */
Blockly.Connection.prototype.isConnectionAllowed = function(candidate) {
    ...
    case Blockly.OUTPUT_VALUE: {
        ...
        if ((candidate.isConnected() && !candidate.targetBlock().isInsertionMarker()) || this.isConnected()) {
            return false;
        }
        break;
    }
    case Blockly.INPUT_VALUE: {
        ...
        if (candidate.isConnected() && !candidate.targetBlock().isMovable() && !candidate.targetBlock().isShadow()) {
            return false;
        }
        break;
    }
    case Blockly.NEXT_STATEMENT: {
        ...
        if (candidate.isConnected() && !this.sourceBlock_.nextConnection && !candidate.targetBlock().isShadow() && candidate.targetBlock().nextConnection) {
            return false;
        }
        break;
    }
 ...
 * 
 * @param {Blockly.Connection}
 *            first The first connection to update.
 * @param {Blockly.Connection}
 *            second The second connection to update.
 * @private
 */
 ...
 * connections that will accept the orphaned block. If at any point there are
 * zero or multiple eligible connections, returns null. Otherwise returns the
 * only input on the last block in the chain. Terminates early for shadow
 * blocks.
 * 
 * @param {!Blockly.Block}
 *            startBlock The block on which to start the search.
 * @param {!Blockly.Block}
 *            orphanBlock The block that is looking for a home.
 ...
 * 
 * @param {!Blockly.Block}
 *            parentBlock The superior block.
 * @param {!Blockly.Block}
 *            childBlock The inferior block.
 * @protected
 */
 ...
 * 
 * @return {Array} List of compatible value types. Null if all types are
 *         compatible.
 * @public
 */
 ...
 * 
 * Headless configurations (the default) do not have neighboring connection, and
 * always return an empty list (the default). {@link Blockly.RenderedConnection}
 * overrides this behavior with a list computed from the rendered positioning.
 * 
 * @param {number}
 *            _maxLimit The maximum radius to another connection.
