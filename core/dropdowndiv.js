 ...
 *               The drop-down can be kept inside the workspace, animate in/out,
 *               etc.
...
/**
 * Drop-downs will appear within the bounds of this element if possible. Set in
...
/**
 * Arrow size in px. Should match the value in CSS (need to position
 * pre-render).
 * 
 * @type {number}
 * @const
...
/**
 * Timer for animation out, to be cleared if we need to immediately hide without
 * disrupting new shows.
 * 
 * @type {number}
 */
...
Blockly.DropDownDiv.createDom = function() {
    ...
    Blockly.DropDownDiv.DIV_.style.transition = 'transform ' + Blockly.DropDownDiv.ANIMATION_TIME + 's, ' + 'opacity ' + Blockly.DropDownDiv.ANIMATION_TIME
            + 's';
};

/**
 * Set an element to maintain bounds within. Drop-downs will appear within the
 * box of this element if possible.
 * 
 * @param {Element}
 *            boundsElement Element to bind drop-down to.
 */
 ...
 * 
 * @param {string}
 *            backgroundColour Any CSS colour for the background.
 * @param {string}
 *            borderColour Any CSS colour for the border.
 */
 ...
 * 
 * @param {string}
 *            category The new category for the drop-down.
 */
...
/**
 * Shortcut to show and place the drop-down with positioning determined by a
 * particular block. The primary position will be below the block, and the
 * secondary position above the block. Drop-down will be constrained to the
 * block's workspace.
 * 
 * @param {!Blockly.Field}
 *            field The field showing the drop-down.
 * @param {!Blockly.Block}
 *            block Block to position the drop-down around.
 * @param {Function=}
 *            opt_onHide Optional callback for when the drop-down is hidden.
 * @param {number}
 *            opt_secondaryYOffset Optional Y offset for above-block
 *            positioning.
 * @return {boolean} True if the menu rendered below block; false if above.
 */
Blockly.DropDownDiv.showPositionedByBlock = function(field, block, opt_onHide, opt_secondaryYOffset) {
...
/**
 * Shortcut to show and place the drop-down with positioning determined by a
 * particular field. The primary position will be below the field, and the
 * secondary position above the field. Drop-down will be constrained to the
 * block's workspace.
 * 
 * @param {!Object}
 *            owner The object showing the drop-down.
 * @param {Function=}
 *            opt_onHide Optional callback for when the drop-down is hidden.
 * @param {number=}
 *            opt_secondaryYOffset Optional Y offset for above-block
 ...
 * @return {boolean} True if the menu rendered below block; false if above.
 */
Blockly.DropDownDiv.showPositionedByField = function(owner, opt_onHide, opt_secondaryYOffset) {
    ...
    Blockly.DropDownDiv.setBoundsElement(owner.getSourceBlock().workspace.getParentSvg().parentNode);
    return Blockly.DropDownDiv.show(owner, primaryX, primaryY, secondaryX, secondaryY, opt_onHide);
};

/**
 * Show and place the drop-down. The drop-down is placed with an absolute
 * "origin point" (x, y) - i.e., the arrow will point at this origin and box
 * will positioned below or above it. If we can maintain the container bounds at
 * the primary point, the arrow will point there, and the container will be
 * positioned below it. If we can't maintain the container bounds at the primary
 * point, fall-back to the secondary point and position above.
 * 
 * @param {Object}
 *            owner The object showing the drop-down
 * @param {number}
 *            primaryX Desired origin point x, in absolute px
 * @param {number}
 *            primaryY Desired origin point y, in absolute px
 * @param {number}
 *            secondaryX Secondary/alternative origin point x, in absolute px
 * @param {number}
 *            secondaryY Secondary/alternative origin point y, in absolute px
 * @param {Function=}
 *            opt_onHide Optional callback for when the drop-down is hidden
...
Blockly.DropDownDiv.show = function(owner, primaryX, primaryY, secondaryX, secondaryY, opt_onHide) {
    ...
    // Update arrow CSS.
    Blockly.DropDownDiv.arrow_.style.transform = 'translate(' + metrics.arrowX + 'px,' + metrics.arrowY + 'px) rotate(45deg)';
    ...
    Blockly.DropDownDiv.positionInternal_(metrics.initialX, metrics.initialY, metrics.finalX, metrics.finalY);
...
/**
 * Helper to position the drop-down and the arrow, maintaining bounds. See
 * explanation of origin points in Blockly.DropDownDiv.show.
 * 
 * @param {number}
 *            primaryX Desired origin point x, in absolute px
 * @param {number}
 ...
 * @param {number}
 *            secondaryY Secondary/alternative origin point y, in absolute px
 * @return {Object} Various final metrics, including rendered positions for
 *         drop-down and arrow.
 */
Blockly.DropDownDiv.getPositionMetrics = function(primaryX, primaryY, secondaryX, secondaryY) {
    ...
    renderX = Blockly.utils.math.clamp(boundsInfo.left, renderX, boundsInfo.right - divSize.width);
    ...
    absoluteArrowX = Blockly.utils.math.clamp(boundsInfo.left, absoluteArrowX, boundsInfo.right);
    ...
    // of a rounded border.
    relativeArrowX = Blockly.utils.math.clamp(Blockly.DropDownDiv.ARROW_HORIZONTAL_PADDING, relativeArrowX, divSize.width
 ...
 * 
 * @param {Object}
 *            owner Object which must be owning the drop-down to hide.
 ...
 * 
 * @param {number}
 *            initialX Initial Horizontal location (window coordinates, not
 *            body).
 * @param {number}
 *            initialY Initial Vertical location (window coordinates, not body).
 * @param {number}
 *            finalX Final Horizontal location (window coordinates, not body).
 * @param {number}
 *            finalY Final Vertical location (window coordinates, not body).
 * @private
 */
...
Blockly.DropDownDiv.repositionForWindowResize = function() {
    ...
    if (Blockly.DropDownDiv.owner_) {
        ...
        var scale = block.workspace.scale;
        var bBox = {
            width : Blockly.DropDownDiv.positionToField_ ? Blockly.DropDownDiv.owner_.size_.width : block.width,
            height : Blockly.DropDownDiv.positionToField_ ? Blockly.DropDownDiv.owner_.size_.height : block.height
        };
        bBox.width *= scale;
        bBox.height *= scale;
        var position = Blockly.DropDownDiv.positionToField_ ? Blockly.DropDownDiv.owner_.fieldGroup_.getBoundingClientRect()
