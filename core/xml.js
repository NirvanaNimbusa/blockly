 ...
 * @param {boolean=}
 *            opt_noId True if the encoder should skip the block IDs.
 ...
 * 
 * @param {!Array.
 *            <!Blockly.VariableModel>} variableList List of all variable
 *            models.
 ...
 * @param {boolean=}
 *            opt_noId True if the encoder should skip the block ID.
 ...
 * 
 * @param {!Blockly.Block}
 *            block A block with fields to be encoded.
 * @param {!Element}
 *            element The XML element to which the field DOM should be attached.
 * @private
 */
...
Blockly.Xml.blockToDom = function(block, opt_noId) {
    var element = Blockly.utils.xml.createElement(block.isShadow() ? 'shadow' : 'block');
    ...
    if (block.inputsInline != undefined && block.inputsInline != block.inputsInlineDefault) {
    ...
    while (node) {
        ...
        if (node.firstChild) {
            ...
            while (node && !node.nextSibling) {
                ...
                if (textNode.nodeType == Blockly.utils.dom.Node.TEXT_NODE && textNode.data.trim() == '' && node.firstChild != textNode) {
...
/**
 * Clear the given workspace then decode an XML DOM and create blocks on the
 * workspace.
 * 
 * @param {!Element}
 *            xml XML DOM.
        ...
        for (var i = 0; i < childCount; i++) {
            ...
            if (name == 'block' || (name == 'shadow' && !Blockly.Events.recordUndo)) {
                ...
                if (variablesFirst) {
                    ...
                    throw Error('\'variables\' tag must exist once before block and ' + 'shadow tag elements in the workspace XML, but it was found in '
                            + 'another location.');
                }
                variablesFirst = false;
            }
...
/**
 * Decode an XML DOM and create blocks on the workspace. Position the new blocks
 * immediately below prior blocks, aligned by their starting edge.
 * 
 * @param {!Element}
 *            xml The XML DOM.
 * @param {!Blockly.Workspace}
 *            workspace The workspace to add to.
 * @return {Array.<string>} An array containing new block IDs.
 */
 ...
 * 
 * @param {!Element}
 *            xmlVariables List of XML variable elements.
 * @param {!Blockly.Workspace}
 *            workspace The workspace to which the variable should be added.
 */
 ...
 * 
 * @param {!Blockly.Block}
 *            block The block that is currently being deserialized.
 * @param {string}
 *            fieldName The name of the field on the block.
 * @param {!Element}
 *            xml The field tag to decode.
 * @private
 */
